...
def request_decisions(batch, model, base_url, api_key):
    papers = [
        {
            "id": item["id"],
            ...
            if missing:
                ...
                for item in batch:
                    if item["id"] not in missing:
                        continue
                    recovered, recovered_usage = request_decisions([item], model, base_url, api_key)
...
def evaluate_candidates(candidates, model, base_url, api_key, batch_size, max_workers):
    batches = [candidates[start:start + batch_size] for start in range(0, len(candidates), batch_size)]
    decisions = []
    usage = {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(request_decisions, batch, model, base_url, api_key): index
        ...
        for future in as_completed(futures):
            ...
            print(f"Completed research filter batch {futures[future] + 1}/{len(batches)}", flush=True)
    return decisions, usage
...
def main():
    ...
    decisions, usage = evaluate_candidates(
        candidates,
        model,
