<div id=toc></div>

# Table of Contents

- [cs.SE](#cs.SE) [Total: 15]
- [cs.AI](#cs.AI) [Total: 22]


<div id='cs.SE'></div>

# cs.SE [[Back]](#toc)

### [1] [Falsifiable Release Gates for Self-Improving Systems](https://arxiv.org/abs/2607.13070)
*Deepak Soni*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出一种通过可证伪发布门和机器验证来确保自我改进代理运行时安全的方法，并在Antahkarana开源运行时中实现，通过七个门和穷举状态空间检查来保证每次变更都符合安全不变性。


<details>
  <summary>Details</summary>
Motivation: 现有安全声明缺乏可验证性，依赖自我评分，需要一种可重复、机器可验证的方法来确保自我改进代理运行时在每次新能力发布时都满足安全约束。

Method: 设计可证伪的发布门系统，每个门包含预定义的机器可验证验收套件；结合有界模型检查，对运行时协调骨架的百万级状态空间进行穷举验证；动作必须由控制环铸造的安全令牌授权，并针对执行轨迹重检查；采用正向约束，收紧变化自动应用，放宽变化需人工审批。

Result: 在Antahkarana运行时上实现了七个门，发布了验收测量结果，证明该方法能有效检测错误（最短反例），且门套件可复现并适用于其他代理框架，单个命令中央非绕过可在秒级完成。

Conclusion: 该方法为自我改进代理运行时提供了可证伪、可复现的安全保证机制，通过有界模型检查与发布门结合，确保每次能力更新都经过严格验证，同时开源实现以促进社区验证和采用。

中文摘要：关于自我改进代理运行时的安全声明几乎都是自我评分的：一个策略文件、一个护栏或一个README承诺。我们描述了可证伪的发布门，以及构建和验证此类系统的方法，使得每个新能力在发布前必须通过一个预先指定的、机器可验证的验收套件，并且在每个门处保持一组固定的常驻不变性。我认为我们在Antahkarana（一个开放运行时）中应用了该方法，通过七个门从基本可观测性进入一个自我治理循环，该循环建议对其自身策略进行更改。没有动作能到达执行器，除非由控制环铸造的安全关键属性能力令牌在经过有界模型的一百万记录可达状态空间上进行穷举机器检查，并针对执行轨迹重新检查。一个故意损坏的模型给出了最短反例，因此检查器是有效的、明显的。自我增强循环受到正向约束：整个写表面是策略规则，收紧变化可自动应用，放宽变化始终需要人工合并，并且自动关闭的提案是指错误预测自身效果的提案；我们发布了全部七个门的验收测量结果，明确定义每个声明的范围（协调骨架的有界模型，而非学习组件），并开源运行时、命令行工具和门套件，以便结果可复现，且门可以针对其他代理框架运行。评审人员可以在几秒钟内重复单个命令中央非绕过。

Original abstract: Safety claims on self-improving agent runtimes are almost always self-graded: a policy file, a guardrail, or a README commitment. We describe falsifiable release gates, and a methodology to build and validate such systems, such that every new capability must pass a pre-specified, machine-verifiable acceptance suite before it ships, and a fixed set of standing invariants is preserved at each gate. I think we applied the method in the Antahkarana, an open runtime, via seven gates from basic observability into a self governing loop that suggests changes to its own policy. no action goes to an effector without a safety-critical property capability token minted by a control ring is exhaustively machine-checked over the one million recorded reachable state space of a bounded model and re-checked against execution traces. A purposely broken model gives the shortest counterexample, so the checker has teeth, is apparent. The self-enhancement loop is positively Constrained: the entire write surface is policy rules, tightening changes. may auto-apply loosening changes always require a human merge and a proposal autoclosed is one that mispredicts its own effect; We publish the acceptance measured results for all the seven gates, define precisely the scope of each claim (a bounded of the coordination skeleton (not the learned components) and free the runtime, either command line tools and the gate suite, so the results reproduce, and gates can run against other agent frameworks. Reviewers may repeat the single command central non-bypass in seconds.

</details>


### [2] [Compaction as Epistemic Failure: How Agentic LLM Tools Fabricate Confirmed Results from Killed Processes](https://arxiv.org/abs/2607.13071)
*Hiroki Tamba*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本文揭示了Claude Code等Agentic LLM编码工具将超时命令的未完成输出错误地记录为确认结果，导致假阳性跨会话传播，原因是混淆了终端输出与持久存储信息。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于发现代理型LLM编码工具在压缩会话历史时存在可靠性缺陷，可能导致后续任务基于错误假设进行，影响数据处理的准确性和自动化流程的可靠性。

Method: 通过分析Claude Code中压缩摘要的生成机制，识别超时命令输出被错误记录的模式，并对比观察与持久存储的差异。

Result: 发现超时命令的退出码143部分输出被当作完整结果记录，假阳性跨会话传播，模型版本间不重新验证，且该问题源于观察与持久性的混淆。

Conclusion: 代理型LLM工具在报告自身操作结果时存在类似LLM-as-judge的可靠性缺陷，这对依赖会话连续性的工作流构成直接威胁，需要改进摘要生成机制以确保信息的可验证性。

中文摘要：代理型LLM编码工具将长时间的会话历史压缩为紧凑摘要，后续会话将其继承为真实情况。本文记录了Claude Code中的一个失败模式：超时命令（退出码143）的部分标准输出被记录在压缩摘要中作为确认结果，导致假阳性结果跨会话和模型版本传播而不重新验证。其根本机制是观察和持久性的混淆，即终端中出现的信息被视为与写入持久存储的信息等价。这一发现扩展了先前关于LLM自我评估失败的分析，这些失败在LLM作为评委的非确定性分级中有所报道，表明代理工具在报告自身操作结果时表现出类似的可靠性缺陷。该失败对任何依赖代理会话连续性的工作流（如数据处理、科学计算或多步骤自动化）都有直接影响。

Original abstract: Agentic LLM coding tools compress long session histories into compaction summaries that subsequent sessions inherit as ground truth. This paper documents a failure mode in Claude Code where partial standard output from timed-out commands (exit code 143) is recorded in compaction summaries as confirmed results, propagating false positives across sessions and model versions without re-verification. The underlying mechanism is a conflation of observation and persistence, where information that appeared in the terminal is treated as equivalent to information written to durable storage. This finding extends the analysis of LLM self-evaluation failures reported in prior work on non-determinism in LLM-as-judge grading by showing that agentic tools exhibit analogous reliability deficits when reporting on their own operational outcomes. The failure has direct implications for any workflow that relies on agentic session continuity for data processing, scientific computation, or multi-step automation.

</details>


### [3] [Inference Economics of Enterprise Coding Agents: A Case Study of Cloud vs. On-Premise LLMs](https://arxiv.org/abs/2607.13080)
*Sheng-Wei Peng,Yi-Hsun Lin,Yi-Pei Lee*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 通过纵向案例研究比较API模型与本地量化模型在编码代理中的成本与质量权衡，发现提示缓存大幅降低API成本，但本地模型缺陷修复率更高；共享GPU下本地部署节省TCO，但以开发者体验为代价。


<details>
  <summary>Details</summary>
Motivation: 工程组织需要在API模型的高推理成本与本地模型的质量损失之间做出选择，本研究旨在通过实证量化这一权衡。

Method: 对单一开发者在两个连续28天周期内分别使用API模型（Claude Code+Claude Opus）和本地量化模型（Opencode+GLM-5.1/5.2）的单仓库生产环境进行非随机化纵向案例研究，分析LLM遥测和Git历史。

Result: API提示缓存（99.3%命中率）使实际成本降低88.6%至$0.57/百万token；本地模型修复提交比率74.9% vs API 45.9%，修复几率高2.6-4.9倍；共享GPU下本地部署节省40.1% TCO，专用则贵43.8%。

Conclusion: 共享GPU下本地部署在成本上有优势，但以更高的缺陷率和更差的开发者体验为代价；混合路由网关存在成本-质量权衡，而非完全优于API基线。

中文摘要：自主编码代理迫使工程组织在基于API的尖端模型（强大的推理能力，高token成本）和本地量化的开放权重模型（承诺低边际成本扩展和数据主权，但推理保真度有所损失）之间做出选择。我们通过一个单一开发者、非随机化的纵向案例研究，在连续两个28天周期内，在一个生产单仓库上研究了这一权衡：基于API的Claude Opus 4.7/4.8配置使用Claude Code，与基于本地的GLM-5.1/5.2配置使用Opencode（量化为NVFP4，在NVIDIA Blackwell硬件上）进行比较。通过分析LLM遥测和Git历史，我们发现提示缓存（99.3%命中率）将实际API成本削减了88.6%，降至有效每百万token 0.57美元——甚至低于共享本地分片的摊销单位成本2.83美元（这是一个利用率依赖的反转；实际总支出和总拥有成本（TCO）是稳健的量）。在可比的代码变更量下，本地配置与更高的缺陷修复负担相关：修复提交比率（FCR）为74.9%对45.9%，在每个难度层级中，提交是修复的几率高出2.6到4.9倍（Mantel-Haenszel OR = 3.61）。在台湾市场参数和对称劳动力模型下，本地部署在共享GPU分配下仍节省了40.1%的真实TCO，而专用预留比缓存API成本高出43.8%。在共享分配下，真正的代价不是金钱，而是可衡量的开发者体验负担——时间戳指标显示更多工作陷入调试循环和更慢的提交节奏——并且离线回放显示混合路由网关沿着成本-质量前沿交易缺陷率与基础设施节省，而非主导纯API基线。

Original abstract: Autonomous coding agents force engineering organizations to choose between API-based frontier models -- strong reasoning at high token cost -- and on-premise quantized open-weights models, which promise low-marginal-cost scaling and data sovereignty at some loss of reasoning fidelity. We study this trade-off through a single-developer, non-randomized longitudinal case study over two contiguous 28-day periods on a production monorepo: an API-based Claude Opus 4.7/4.8 configuration using Claude Code versus an on-premise GLM-5.1/5.2 configuration using Opencode, quantized to NVFP4, on NVIDIA Blackwell hardware. Analyzing LLM telemetry and Git history, we find that prompt caching (99.3% hit rate) cuts realized API cost by 88.6% to an effective \$0.57 per million tokens -- below even the \$2.83 amortized unit cost of the shared on-premise slice (a utilization-dependent inversion; total realized spend and total cost of ownership (TCO) are the robust quantities). At comparable gross code churn, the local configuration was associated with a far higher defect-repair burden: a Fix Commit Ratio (FCR) of 74.9% versus 45.9%, with the odds of a commit being a repair 2.6 to 4.9 times higher within every difficulty tier (Mantel-Haenszel OR = 3.61). Under Taiwan-market parameters and a symmetric labor model, on-premise deployment nonetheless saves 40.1% of true TCO under shared GPU allocation, whereas dedicated reservation costs 43.8% more than the cached API. Under shared allocation, the genuine penalty is not monetary but a measurable developer-experience burden -- timestamp indicators show more work trapped in debugging spirals and a slower commit cadence -- and an offline replay shows hybrid routing gateways trade defect rate for infrastructure savings along a cost-quality frontier rather than dominate the pure-API baseline.

</details>


### [4] [Self-Improving AI Coding Agents Through Accumulated Behavioral Rules: A Closed-Loop Framework](https://arxiv.org/abs/2607.13091)
*Aditya Aggarwal,Nahid Farhady Ghalaty*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出一个闭环框架，将人工审查评论编码为持久行为规则，使编码代理在不更新权重的情况下通过积累规则实现跨会话错误自检，并在实际微服务平台上验证了0%重复率和审查焦点转移。


<details>
  <summary>Details</summary>
Motivation: LLM编码代理重复相同错误，缺乏保留人工审查反馈的机制，导致无法在多次会话中持续改进。

Method: 构建闭环框架：将每次接受的审查评论编码为持久规则，存储在版本控制指令文件中；在代码提交前执行自检清单；自动化验证规则集完整性。规则集随反馈增长。

Result: 在35+服务微服务平台部署，规则集从5条增至18条行为规则及15+语言标准；11个工作会话中，规则禁止的错误类别复发率为0%，审查从低层正确性转向设计验证，且规则可跨代理接口转移。

Conclusion: 该框架实现了编码代理的持久跨会话学习，无需更新模型权重，通过积累工程智慧持续改进，优于现有方法（如Reflexion等）。
}

中文摘要：基于LLM的编码助手在多次会话中重复相同的错误类别，因为它们缺乏一种机制来保留来自人工审查反馈的修正。我们提出了一个闭环框架，其中每个被接受的审查评论都被编码为一条持久的行为规则，逐步扩展代理能够自我检测的错误类别的集合。该框架结合了一个版本控制的指令文件中的累积规则集、在代码提交前执行的自我审查清单，以及自动化验证以确保规则集在增长时保持完整性。在一个包含35+个服务的微服务平台部署中，规则集从5条行为规则增长到18条，加上15+条语言特定标准和一份15项的自我审查清单，所有这些都来自真实的审查反馈。我们呈现了来自11个记录的工作会话的经验结果，涵盖代码生成、PR审查、事件调查和跨服务重构。我们观察到，积累的规则将审查工作从低层正确性转向设计级验证，实现了对规则禁止的错误类别0%的重复率，并且跨异构代理接口转移。我们比较了我们的方法与相关工作在经验性LLM学习（Reflexion, ExpeL, Voyager）和自动化代码审查（CodeReviewer, SWE-bench代理）方面的差异，表明我们的框架在不更新权重的情况下实现了持久的跨会话学习，在生成代码库而非合成基准上操作，并解决了现有基准未测量的一个正交维度（随时间的行为一致性）。结果是一个编码代理，它在每次审查循环中改进，积累其人类协作者的工程智慧，而不改变一个模型权重。

Original abstract: LLM-based coding agents repeat the same classes of mistakes across sessions because they lack a mechanism to retain corrections from human review feedback. We present a closed-loop framework in which every accepted review comment is codified as a persistent behavioral rule, progressively expanding the set of error classes the agent can self-detect. The framework combines an accumulating rule set in a version-controlled instruction file, a self-review checklist executed before code submission, and automated validation that ensures rule set integrity as it grows. In deployment across a 35+ service microservices platform, the rule set grew from 5 to 18 behavioral rules, 15+ language-specific standards, and a 15-item self-review checklist, all derived from real review feedback. We present empirical results from 11 recorded working sessions spanning code generation, PR review, incident investigation, and cross service refactoring. We observe that accumulated rules shift review effort from low-level correctness toward design-level validation, achieve a measured 0% recurrence rate for ruled-against error classes, and transfer across heterogeneous agent interfaces. We compare our approach against related work in experiential LLM learning (Reflexion, ExpeL, Voyager) and automated code review (CodeReviewer, SWE-bench agents), showing that our framework achieves persistent cross-session learning without weight updates, operates on production codebases rather than synthetic benchmarks, and addresses an orthogonal dimension (behavioral consistency over time) that existing benchmarks do not measure. The result is a coding agent that improves with every review cycle, accumulating the engineering wisdom of its human collaborators without changing a single model weight.

</details>


### [5] [SemaDiff: Identifying Semantic-Changing Commits with Generated Code and Tests](https://arxiv.org/abs/2607.13111)
*Maha Ayub,Michael Konstantinou,Ahmed Khanfir,Nikolaos Tsantalis,Mike Papadakis*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: SemaDiff通过行为分析（比较提交前后版本的测试执行）和LLM生成测试代码，准确识别语义保持提交，在183个Java提交数据集上达到76%准确率和100%语义更改检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法确保提交是纯粹语义保持的（无行为更改交织），影响调试、故障定位等任务。

Method: 分析提交差异，提取修改代码及其依赖，使用LLM生成依赖类的调用方法，自动生成测试，比较两个版本测试结果是否一致。

Result: 在183个Java提交数据集上，SemaDiff区分语义保持/更改的准确率约76%，语义更改检测精度100%。

Conclusion: SemaDiff能有效区分语义保持与更改提交，但仍有提升空间，未来可改进测试生成和依赖分析。

中文摘要：区分语义保持提交与语义更改提交仍然是软件仓库挖掘中的一个开放挑战。虽然现有方法能够准确检测重构提交，但它们无法确保提交是纯粹语义保持的，没有交织任何行为更改的修改。这一限制可能影响多个任务，例如调试、故障定位、错误数据集构建、回滚分析和错误修复的回溯。为了填补这一空白，我们提出了SemaDiff，一种通过基于行为的分析来识别语义保持提交的新方法：对提交前后版本执行相似测试的比较。由于受重构影响的代码通常难以测试，并且在两个版本之间不同，我们提议为这些代码生成额外的调用方法，作为测试目标。给定一个提交，SemaDiff分析差异以识别修改后的代码，并提取调用它的未更改依赖代码。然后，它使用大型语言模型生成一个额外的依赖类，以在两个版本中执行更改后的代码，并自动为依赖代码生成测试。这样，我们为不同的代码版本获得了相同的测试，从而能够检测行为差异。仅当所有生成的测试在两个版本上产生相同结果时，该提交才被分类为语义保持。为了评估SemaDiff，我们构建并手动标注了一个包含183个提交的数据集，这些提交来自知名的开源Java项目。获得的结果表明，SemaDiff在大约76%的情况下准确地区分了语义保持提交和语义更改提交，并且在语义更改提交检测中达到了100%的精确度。

Original abstract: Distinguishing semantic-preserving commits from changing ones remains an open challenge in software repository mining. While existing approaches detect refactoring commits accurately, they cannot ensure that a commit is purely semantic-preserving, without any interleaving behaviour-changing modification. This limitation can impact several tasks, such as debugging, fault localisation, bug dataset construction, rollback analysis, and bug fixes backporting. To fill this gap, we propose SemaDiff, a novel approach for identifying semantic-preserving commits through behaviour-based analysis; comparison of similar test execution on pre- and post-commit versions. As code impacted by the refactoring is often hard to test and different accross both versions, we propose generating additional calling methods to that code, which serve as testing target. Given a commit, SemaDiff analyses the diff to identify modified code and extracts unchanged dependent code that calls it. It then generates an additional dependent class using a large language model to exercise the changed code in both versions, and automatically generates tests for the dependent code. This way, we obtain the same tests for the different code versions, enabling the behavioural-difference detection. The commit is classified as semantic-preserving only if all generated tests produce identical outcomes across the two versions. To evaluate SemaDiff, we construct and annotate manually a dataset of 183 commits, gathered from well-known open-source Java projects. The obtained results show that SemaDiff distinguishes accurately semantic-preserving from -- changing commits in about 76% of the cases, with a 100% precision in semantic-changing commit detection.

</details>


### [6] [Design-System-Aware Development with AI: Evaluating Productivity and Design Consistency](https://arxiv.org/abs/2607.13156)
*Luciane Silva,Thayssa Rocha,Nicole Davila,Gustavo Pinto*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 实验表明，DS感知的AI辅助开发能显著减少前端开发时间（46.7%-69.4%）、提高任务完成度并降低性能变异性。


<details>
  <summary>Details</summary>
Motivation: 设计系统有助于标准化前端开发，但开发人员仍面临从高保真模型到一致性界面的挑战，而AI辅助工具在该领域缺乏实证证据。

Method: 在巴西大型企业进行受控实验，比较手动开发、仅DS开发和DS感知AI辅助开发，覆盖Angular、iOS和Android技术栈，包含两个实验周期。

Result: AI辅助开发显著缩短交付时间（46.7%至69.4%），提高任务完成度，降低性能变异性，并减少工作流摩擦。

Conclusion: DS感知的AI工具能加速开发、提高设计保真度，为工业前端工作流带来实际效益。

中文摘要：设计系统（DS）有助于标准化前端开发，但开发人员在将高保真模型转换为一致且生产就绪的界面时仍面临挑战。尽管AI辅助工具已成为一种潜在解决方案，但关于其在以DS为中心的工作流程中有效性的实证证据仍然有限。本文报告了一项在巴西一家大型企业进行的受控实验，比较了手动开发、仅使用DS开发以及DS感知的AI辅助开发在Angular、iOS和Android技术栈上的表现。两个实验周期的结果表明，AI辅助显著缩短了交付时间（46.7%至69.4%），提高了任务完成度，并降低了性能变异性。对中断模式的分析进一步表明工作流摩擦减少和任务执行更顺畅。这些发现提供了实证证据，表明DS感知的AI工具可以显著加速开发、提高设计保真度，并为工业前端工作流带来实际效益。

Original abstract: Design Systems (DS) help standardize front-end development, yet developers still face challenges when translating high-fidelity mockups into consistent, production-ready interfaces. Although AI-assisted tools have emerged as a potential solution, empirical evidence on their effectiveness within DS-centered workflows remains limited. This paper reports a controlled experiment conducted at a large Brazilian enterprise that compares manual development, DS-only development, and DS-aware AI-assisted development across Angular, iOS, and Android stacks. Results from two experimental cycles show that AI assistance significantly reduced time-to-delivery (by 46.7% to 69.4%), increased task completeness, and decreased performance variability. Analysis of break patterns further suggests reduced workflow friction and smoother task execution. These findings provide empirical evidence that DS-aware AI tools can significantly accelerate development, improve design fidelity, and yield practical benefits for industrial front-end workflows.

</details>


### [7] [From Human-Centric to Agentic Code Review: The Impact of Different Generations of Generative AI Technology on Review Quality](https://arxiv.org/abs/2607.13196)
*Suzhen Zhong,Shayan Noei,Bram Adams,Ying Zou*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 通过对207个GitHub项目的102万次拉取请求分析，发现AI代理参与能提高审查效率，但未提升质量，人机协作模式是效率的关键因素。


<details>
  <summary>Details</summary>
Motivation: 缺乏关于从人工审查向AI支持审查转变如何影响效率和质量的实证证据。

Method: 将审查讨论建模为交互序列，识别AI采用实践，分析协作模式与效率、质量的关系。

Result: AI代理发起的或涉及多个代理的审查模式与更快决策相关，但未提升质量；人机协作模式成为效率最强解释因素。

Conclusion: 为设计兼顾效率与质量的AI支持代码审查流程提供实证指导。

中文摘要：代码审查有助于在代码集成前维护软件质量，但也给人工审查者带来了沉重负担。随着生成式人工智能成为软件开发的一部分，代码审查正从主要的人工审查过程转向AI支持的审查过程，其中大语言模型（LLM）审查者和AI代理审查者与人工审查者一同参与。然而，我们仍缺乏关于这种转变如何影响审查效率和审查质量的实证证据。在本文中，我们研究了来自207个GitHub项目的102万次已审查拉取请求，这些项目经历了三个代码审查时代：以人为中心的审查、LLM辅助审查和代理代码审查。我们发现了三种AI审查者采用实践：渐进式AI采用、快速LLM采用和快速AI代理采用。我们进一步将拉取请求审查讨论建模为审查者交互序列，以描述人工、LLM和AI代理审查者在审查过程中的协作方式。我们的结果表明，涉及代理的协作模式，特别是由AI代理发起或涉及多个AI代理的审查，与渐进式AI采用和快速AI代理采用下的更快审查决策相关。然而，这些效率提升并未转化为更好的审查质量。我们还发现，审查活动和拉取请求类型在所有时代中仍然重要，而一旦LLM和AI代理审查者参与，人机协作模式成为审查效率的最强解释因素。这些发现为设计提高效率而不削弱审查质量的AI支持代码审查流程提供了实证指导。

Original abstract: Code review helps maintain software quality before code integration, but it also imposes a substantial workload on human reviewers. As generative artificial intelligence becomes part of software development, code review is shifting from a primarily human review process toward AI-supported review processes in which large language model (LLM) reviewers and AI agent reviewers participate alongside human reviewers. However, we still lack empirical evidence on how this transition affects review efficiency and review quality. In this paper, we study 1.02 million reviewed pull requests from 207 GitHub projects that transition across three code review eras: human-centric review, LLM-assisted review, and agentic code review. We identify three AI reviewer adoption practices: Gradual AI Adoption, Rapid LLM Adoption, and Rapid AI Agent Adoption. We further model pull request review discussions as reviewer interaction sequences to characterize how human, LLM, and AI agent reviewers collaborate during the review process. Our results show that agent-involved collaboration patterns, especially reviews initiated by AI agents or involving multiple AI agents, are associated with faster review decisions under Gradual AI Adoption and Rapid AI Agent Adoption. However, these efficiency gains do not translate into better review quality. We also find that review activity and pull request type remain important across eras, while human-AI collaboration patterns become the strongest explanatory factor for review efficiency once LLM and AI agent reviewers participate. These findings provide empirical guidance for designing AI-supported code review processes that improve efficiency without weakening review quality.

</details>


### [8] [Can LLMs Learn and Apply Multi-Level Modelling Semantics? A First Empirical Study](https://arxiv.org/abs/2607.13257)
*Yuhong Fu,Weixing Zhang,Bowen Jiang,Haowei Cheng,Karamjti Kaur,Markus Stumptner*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 首次实证研究三个商业LLM在多级建模任务中的表现，发现句法正确性可行但语义正确性有限，模型擅长复制显式内容但难以推理隐含结构，自我检查作用有限，Claude表现最均衡。


<details>
  <summary>Details</summary>
Motivation: 工业5.0强调以人为本，对建模工具提出更高要求；多级建模（MLM）能表示多个抽象层次但语义约束复杂；LLMs在两级建模中有效，但能否推广到语义不同的MLM未知，故需实证检验。

Method: 使用GPT-5.4、Claude Opus 4.6和Gemini 3.1 Pro，在六种提示策略下为MULTI Warehouse Challenge生成SLICER语言的多级模型，共90个模型，与手动验证参考模型进行14项指标对比。

Result: 句法正确性可及，语义正确性部分实现（实例化/专门化正确性52%-79%）；模型重现显式内容但隐含结构/约束完成度低；提示策略在精确性与完整性间权衡；自我检查主要作为规则检查器，可靠性有限；Claude表现最均衡。

Conclusion: 明确了当前LLM对多级建模的能力边界，为工业5.0中以人为本的、AI辅助的建模工作流设计提供指导。

中文摘要：工业5.0强调以人为中心的工业系统设计，对建模工具提出了额外需求。多级建模（MLM）可以直接表示三个或更多抽象层次，但这以模型正确性所依赖的更复杂的语义约束为代价。大型语言模型（LLMs）在模型驱动工程中得到了越来越多的研究，但这一证据完全基于两级建模任务，其是否推广到语义不同的MLM仍未得到检验。本文提出了对这一问题的首次实证研究。我们让三个商业LLM（GPT-5.4、Claude Opus 4.6和Gemini 3.1 Pro）在六种提示策略下，为MULTI Warehouse Challenge生成SLICER语言的多级模型，共生成90个模型，并与手动验证的参考模型进行14项指标对比。句法正确性在可及范围内，但语义正确性仅部分实现，实例化/专门化正确性在52%到79%之间。模型重现了任务文本中明确陈述的内容，但很少完成文本暗示但未说明的结构和约束。提示策略在精确性和完整性之间权衡，自我检查主要作为规则检查器，而非可靠地改进与参考设计的一致性。在三个LLM中，Claude展现出最均衡的轮廓。这些结果阐明了当前LLM对MLM能力的边界，并为工业5.0中以人为本的、AI辅助的建模工作流设计提供信息。

Original abstract: Industry 5.0 emphasises human-centric industrial system design, placing additional demands on modelling tools. Multi-level modelling (MLM) can directly represent three or more abstraction levels, but this comes at the cost of more complex semantic constraints that model correctness depends on. Large Language Models (LLMs) have been increasingly studied in model-driven engineering, but this evidence rests entirely on two-level modelling tasks, and whether it generalises to MLM, whose semantics differ in kind, remains untested. This paper presents the first empirical study of this question. We have three commercial LLMs (GPT-5.4, Claude Opus 4.6, and Gemini 3.1 Pro) generate multi-level models for the MULTI Warehouse Challenge in the SLICER language under six prompting strategies, yielding 90 generated models compared against a manually validated reference using fourteen metrics. Syntactic correctness is within reach, but semantic correctness is only partially achieved, with Instantiation/Specialisation Correctness ranging from 52% to 79%. Models reproduce content stated explicitly in the task text, but rarely complete structure and constraints the text implies without stating. Prompting strategies trade off precision against completeness, and self-checking functions mainly as a rule checker rather than reliably improving alignment with the reference design. Among the three LLMs, Claude shows the most balanced profile. These results clarify the boundaries of current LLM capability for MLM and inform the design of human-centred, AI-assisted modelling workflows for Industry 5.0.

</details>


### [9] [Faithful Autoformalization of Natural Language Assertions](https://arxiv.org/abs/2607.13303)
*Hongyi Liu,Madhusudan Parthasarathy,Adithya Murali*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: Monty是一个利用LLM和基于测试的有效性及一致性评分来自动从自然语言生成形式化断言的框架，在541个Java任务上比直接使用LLM翻译精度提升最多20点。


<details>
  <summary>Details</summary>
Motivation: 手动编写形式化契约费时易错，LLM自动形式化面临断言有效性期望和自然语言歧义的挑战。

Method: 提出Monty框架，使用LLM生成候选断言，并通过一致性评分度量和基于测试的有效性评分进行过滤。

Result: 在541个Java类断言生成任务上，相比直接使用LLM，精度提升最多20点。

Conclusion: Monty通过基于测试的过滤技术，比直接翻译更可靠地生成正确断言，有效提升形式化自动化的可靠性。

中文摘要：形式化契约对于软件测试和验证至关重要，但编写它们仍然费时且容易出错。LLM提供了一条通往自动形式化的有前景的路径：从自然语言规范合成可执行断言，从而弥合非正式开发者意图与正式可执行规范之间的鸿沟。我们提出了Monty：一个用于断言的自动形式化框架，它解决了断言有效性的期望和自然语言中的歧义带来的挑战。我们的技术基于使用一种新颖的一致性评分度量以及通过测试代码与形式化断言得到的有效性评分来过滤形式化结果。我们在源自22个类似集合的Java类的541个断言生成任务上评估了我们的方法，并表明我们的技术比直接使用LLM翻译断言更可靠地生成真实结果（平均精度提升最多20点）。

Original abstract: Formal contracts are essential for software testing and verification, yet writing them remains labor-intensive and error-prone. LLMs offer a promising path toward autoformalization: synthesizing executable assertions from natural-language specifications and thereby bridging the gap between informal developer intent and formal executable specifications. We present Monty: an autoformalization framework for assertions that tackles the challenges of expectations of validity of assertions and ambiguity in natural-language. Our techniques are based on filtering formalizations using a novel conformance score metric and validity scores obtained from testing the code against formalized assertions. We evaluate our approach on 541 assertion-generation tasks derived from 22 collection-like Java classes, and show that our technique produces the ground truth more reliably (improving upto 20 points in precision on average) than when using LLMs naively to translate assertions.

</details>


### [10] [A Telemetry-Driven Model for Quantifying Upgrade Risk in Durable Workflow Execution](https://arxiv.org/abs/2607.13617)
*Luca Maraschi,Matteo Collina*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出一个概率模型来量化工作流升级风险，仅使用静态差异和现有遥测数据，无需试运行，并能处理运行间耦合。


<details>
  <summary>Details</summary>
Motivation: 持久化工作流引擎在版本升级时面临重放失效问题，现有方法无法处理长时间运行的工作流，需要一种轻量级风险量化方法。

Method: 构建封闭式概率模型，风险分解为后向（重水化）和前向（控制流）两项，通过贝叶斯估计得到带有置信区间的WUR分数，并通过耦合图处理运行间依赖。

Result: 零后向风险可保证安全重水化，并得出迁移/审查/固定策略；考虑耦合时，问题转化为最小割求解。

Conclusion: 该模型能有效量化升级风险，减少对旧版本依赖，支持精细化的运行分类策略。

中文摘要：持久化工作流引擎通过确定性重放不可变事件日志来重构执行状态，将每个进行中的运行与产生其历史的代码版本耦合：新部署可能使在旧版本下启动的运行的重放失效，静默地破坏状态或阻碍进展。现有的缓解措施——固定版本、补丁门控、并行部署——将每次变更视为最大危险并清空旧版本，这对于休眠数周的工作流来说是不可行的。我们提出一个封闭形式的概率模型，仅使用静态结构差异和协议已经持久化的遥测数据（事件日志、步骤负载、历史路径），无需试运行、沙箱或影子执行，即可量化将进行中运行从工作流版本V1升级到V2的风险。风险沿三个轴分解（协议、接口、状态迁移），并结合了一个精确的后向（重水化）项（基于记录前缀模并发完成的迹等价计算）和一个概率前向项（来自经验估计的控制流马尔可夫模型中的命中概率）。估计全程采用贝叶斯方法，因此工作流升级风险（WUR）分数带有可信区间，而稀疏遥测表现为不确定性。我们证明零后向风险判断可保证新版本下的安全重水化，并推导出一个将运行划分为迁移、审查和固定类别的策略。最后我们放弃运行间独立性假设：通过钩子、层次结构和共享资源的耦合通过经验耦合图捕获，舰队风险成为故障传染算子的最小不动点，而考虑耦合的迁移/固定划分精确计算为最小s-t割。

Original abstract: Durable workflow engines reconstruct execution state by deterministically replaying an immutable event log, coupling every in-flight run to the code version that produced its history: a new deployment can invalidate the replay of runs started under the old version, silently corrupting state or halting progress. Existing mitigations -- pinning, patch gates, side-by-side deployment -- treat every change as maximally dangerous and drain old versions, untenable for workflows that sleep for weeks.
  We present a closed-form probabilistic model that quantifies the risk of upgrading in-flight runs from workflow version $V_1$ to $V_2$ using only a static structural diff and telemetry the protocol already persists -- event logs, step payloads, historical paths -- with no dry-run, sandbox, or shadow execution. Risk decomposes along three axes (protocol, interface, state migration) and combines an exact backward (rehydration) term, computed on recorded prefixes modulo trace equivalence of concurrent completions, with a probabilistic forward term from hitting probabilities in an empirically estimated Markov model of control flow. Estimation is Bayesian throughout, so the Workflow Upgrade Risk (WUR) score carries a credible interval and thin telemetry surfaces as uncertainty. We prove that a zero backward-risk verdict certifies safe rehydration under the new version, and derive a policy partitioning runs into migrate, review, and pin classes. Finally we drop the inter-run independence assumption: coupling through hooks, hierarchy, and shared resources is captured by an empirical coupling graph, fleet risk becomes the least fixpoint of a failure-contagion operator, and the coupling-aware migrate/pin partition is computed exactly as a minimum s-t cut.

</details>


### [11] [The Replication Assessment Problem in Software Engineering](https://arxiv.org/abs/2607.13815)
*Giuseppe Destefanis,Martin Shepperd,Leila Yousefi*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 本研究系统回顾了软件工程复制研究的评估实践，发现标准不一致，并提出了一套基于统计原则的评估框架。


<details>
  <summary>Details</summary>
Motivation: 记录当前软件工程复制研究的评估实践，识别因标准不一致导致的问题。

Method: 对2021-2025年的10项复制研究进行系统综述，提取并分析评估标准的异质性和逻辑一致性。

Result: 发现评估实践高度异质，存在矛盾标准，缺乏对测量不确定性的考虑；提出基于统计原则的评估框架。

Conclusion: 采用一致透明的评估原则可减少模糊性，提高可比性，促进可靠证据积累。

中文摘要：背景：软件工程中的复制研究越来越常见，但其解释仍然不确定且不一致，因为评估通常依赖于定义模糊或临时的标准。目的：本研究旨在记录当前经验软件工程中复制研究结果的评估方式，识别因标准不一致而产生的问题，并提出了一个有原则的评估框架。方法：我们对复制研究进行了系统综述，搜索涵盖了近期的经验软件工程复制研究（2021-2025）。对于每项研究，我们提取了用于评估复制结果的标准，并分析了这些标准的异质性、逻辑一致性以及与既定统计原则的一致性。结果：共找到10项复制研究。分析揭示了评估实践中的显著异质性，类似数据应用了矛盾的标准，对测量不确定性的认可有限，且缺乏共同标准。我们提出了一个基于统计、方法和测量考虑的框架，并通过实例演示其应用。结论：采用一致且透明的评估原则将减少模糊性，提高可比性，并支持软件工程复制研究中更可靠的证据积累。

Original abstract: Background: Replication studies in software engineering are increasingly common, yet their interpretation remains uncertain and inconsistent because assessments frequently rely on loosely defined or ad hoc criteria. Aim: This study aims to document how replication study outcomes are currently assessed in empirical software engineering, identify problems arising from inconsistent criteria and propose a principled framework for meaningful evaluation. Method: We conducted a systematic review of replication studies, with the search covering recent empirical software engineering replications (2021--2025). For each study, we extracted the criteria used to assess replication outcomes and analysed these for heterogeneity, logical consistency, and alignment with established statistical principles. Results: A total of 10 replication studies were located. The analysis reveals substantial heterogeneity in assessment practices, with contradictory criteria applied to similar data, limited acknowledgement of measurement uncertainty, and an absence of shared standards. We propose a principled framework grounded in statistical, methodological, and measurement considerations, and demonstrate its application through worked examples. Conclusions: Adopting consistent and transparent assessment principles would reduce ambiguity, improve comparability, and support more reliable evidence accumulation in software engineering replication research.

</details>


### [12] [PROBE: Benchmarking Code Generation in Large Language Models](https://arxiv.org/abs/2607.13820)
*Rodrigo Pato Nogueira,Marco Vieira,João R. Campos*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出PROBE框架，从功能正确性、接近有效解决方案程度和代码质量三个维度评估LLM代码生成，发现模型在困难问题和资源少的语言上表现不佳，且常犯低级错误。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准局限于单一编程语言和单元测试，缺乏对代码质量和接近有效解决方案程度的综合评估。

Method: 构建PROBE基准框架，包含多样化指标、代表性工作负载、多种提示模板和稳健实验程序，从功能正确性、代码质量和接近有效解决方案三个维度评估LLM生成的代码。

Result: LLM在简单任务上表现良好，但在困难问题上表现不佳；小模型在低资源语言上表现更差；常见错误包括基本且易避免的错误。

Conclusion: LLM代码生成虽有一定效果，但不可靠，需要进一步改进以提高稳定性和质量。

中文摘要：大型语言模型（LLM）越来越多地用于日常软件工程任务，特别是在自动化代码生成方面。尽管被广泛采用，但这些模型仍远非完美，因此系统且公平的评估对于理解其优势和局限性至关重要。在代码生成方面，现有基准存在局限性：它们通常针对单一编程语言，并且主要依赖单元测试结果，而忽略了其他关键维度，例如生成代码的整体质量及其与有效解决方案的接近程度。为了解决这些差距，我们引入了PROBE，一个可扩展的基准框架，与先前工作不同，它建立了一个基于多样化且定义明确的指标、代表性工作负载、多种提示模板以及稳健实验程序的系统结构。在实践中，LLM生成的代码沿着三个互补维度进行评估：功能正确性、与有效解决方案的接近程度以及代码质量，从而实现性能的综合评估。我们使用PROBE在三种提示策略下评估了四个开源模型和两个专有模型，涉及五种编程语言。我们进一步通过代码中常见错误的研究补充了这一分析，并提供了具体的例子，从而更清晰地了解LLM容易在哪些方面出错。我们的研究结果表明，虽然LLM取得了有希望的结果，但它们难以处理更困难的问题，并且在较小模型的情况下，难以处理训练可用资源更少的编程语言，并且它们经常由于基本且容易避免的错误而失败，这突显了自动生成代码的不可靠性。

Original abstract: Large Language Models (LLMs) are increasingly being used in everyday software engineering tasks, particularly in automated code generation. Despite their widespread adoption, these models remain far from perfect, making systematic and fair evaluation essential to understand their strengths and limitations. In the context of code generation, existing benchmarks are limited: they often target a single programming language and rely primarily on unit test outcomes, while overlooking other critical dimensions such as the overall quality of the generated code and its closeness to a valid solution. To address these gaps, we introduce PROBE, an extensible benchmark framework that, unlike prior work, establishes a systematic structure built on diverse and well-defined metrics, representative workloads, varied prompt templates, and a robust experimental procedure. In practice, the code generated by the LLMs is evaluated along three complementary dimensions: functional correctness, proximity to valid solutions, and code quality, enabling a comprehensive assessment of performance. We use PROBE to evaluate four open-source and two proprietary models under three prompting strategies across five programming languages. We further complement this analysis with a study of common errors in the code and provide concrete examples, offering clearer insight into where LLMs tend to struggle. Our findings show that, while LLMs achieve promising results, they struggle with harder problems and, in the case of smaller models, with programming languages that have fewer available resources for training, and they often fail due to fundamental and easily avoidable errors that underscore the unreliability of automatically generated code.

</details>


### [13] [ProfMalPlus: Agent-Coordinated Detection of Malicious NPM Packages via Static-Dynamic Analysis Synergy](https://arxiv.org/abs/2607.13965)
*Yiheng Huang,Zhijia Zhao,Bihuan Chen,Susheng Wu,Zhuotong Zhou,Yiheng Cao,Kun Hu,Xin Hu,Xin Peng*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: ProfMalPlus结合对象敏感行为图和LLM推理，有效检测NPM恶意包，F1达98.1%，发现597个未知恶意包。


<details>
  <summary>Details</summary>
Motivation: 现有检测器无法充分建模混淆行为，忽略JavaScript面向对象特性，静动态分析协调不佳，丢失语义信息。

Method: 构建对象敏感行为图，提取安全相关切片，添加静态分析证据，本地LLM判断，自一致性降低方差，全局判断形成判定，未确定时通过第三方丰富或动态增强补充证据，最后定位恶意代码。

Result: F1-score 98.1%，优于现有检测器3.5%-52.6%，发现597个未知恶意包。

Conclusion: ProfMalPlus有效提升NPM恶意包检测性能，发现大量未知威胁。

中文摘要：开源软件容易受到通过传递依赖关系的供应链攻击，尤其是注入到NPM包中的恶意代码。现有的检测器通常无法充分建模混淆行为，忽略JavaScript的面向对象特性，静态和动态分析协调不佳，并且在行为抽象过程中丢失语义信息。我们提出ProfMalPlus，一种恶意NPM包检测器，它将对象敏感行为图与对注释代码片段的协调LLM推理相结合。它识别安装命令和入口文件，然后构建捕获敏感API、第三方调用和未解析调用的图。从这些图中，ProfMalPlus提取与安全相关的片段并添加内联静态分析证据。本地判断代理独立评估每个片段。自一致性合并重复判断以减少LLM方差，而全局判断器综合它们的报告形成入口级判定。对于未确定的案例，路由器选择第三方丰富（添加注册表导出的模块和方法语义）或动态增强（在沙箱中执行包以解决运行时依赖行为）。丰富后的证据被反馈用于重新评估。最后，定位代理报告恶意代码片段及其解释。ProfMalPlus实现了98.1%的F1分数，比最先进的检测器高出3.5%到52.6%。它还识别了597个以前未知的恶意包，全部已确认并从NPM中移除。

Original abstract: Open source software is vulnerable to supply-chain attacks through transitive dependencies, especially malicious code injected into NPM packages. Existing detectors often inadequately model obfuscated behavior, overlook JavaScript's object-centric features, poorly coordinate static and dynamic analysis, and lose semantic information during behavior abstraction. We propose ProfMalPlus, a malicious NPM package detector combining object-sensitive behavior graphs with coordinated LLM reasoning over annotated code slices. It identifies installation commands and entry files, then constructs graphs capturing sensitive APIs, third-party calls, and unresolved calls. From these graphs, ProfMalPlus extracts security-relevant slices and adds inline static analysis evidence. Local judge agents independently assess each slice. Self-consistency consolidates repeated judgements to reduce LLM variance, while a global judge synthesizes their reports into an entry-level verdict. For undetermined cases, a router selects either third-party enrichment, which adds registry derived module and method semantics, or dynamic augmentation, which executes the package in a sandbox to resolve runtime dependent behavior. The enriched evidence is fed back for reassessment. Finally, a localization agent reports malicious code snippets with explanations. ProfMalPlus achieves a 98.1% F1-score, outperforming state-of-the-art detectors by 3.5% to 52.6%. It also identified 597 previously unknown malicious packages, all confirmed and removed from NPM.

</details>


### [14] [Early Adoption of Agentic Coding Tools by GitHub Projects](https://arxiv.org/abs/2607.14037)
*Maliha Noushin Raida,Daqing Hou*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 分析了2,361个GitHub仓库的25,264个智能体PR，发现智能编码工具采纳集中在少数项目，小型项目活动更高，生产力差异大，人机协作以单一人类监督为主。


<details>
  <summary>Details</summary>
Motivation: 先前研究关注PR级结果，但对项目级采纳和管理了解不足，本文旨在填补这一空白。

Method: 从2,361个流行GitHub仓库收集25,264个智能体PR，分析采纳率、生产力和协作模式。

Result: 中位仓库三个月仅生成1-2个智能体PR；小型项目参与率和活动更高；生产力差异大，多数项目低于36 PR/参与者的阈值；协作以单一人类监督为主。

Conclusion: 智能体贡献的整合不仅依赖能力提升，还需人类和组织过程，未来需跟踪采纳演进。

中文摘要：智能编码工具日益能够生成并向软件项目提交拉取请求（PR），为软件开发引入了新形式的人机协作。虽然先前的研究已经考察了智能体生成贡献的PR级结果，但对于智能编码工具在项目层面如何被采纳和管理知之甚少。在本文中，我们分析了来自2,361个流行GitHub仓库的25,264个智能体PR，以研究（1）智能编码工具的采纳情况，（2）项目级智能体PR生产力，以及（3）人机协作模式。我们的结果表明，中位仓库在三个月期间仅生成一到两个智能体PR，这表明密集采纳仍集中在少数项目中。同时，小型项目（1-5个贡献者）的参与率和智能体PR活动平均水平高于中型和大型项目。我们还观察到项目级智能体PR生产力存在显著差异。虽然少数项目在三个月观察期内超过了行业报告的每个参与者36个PR的估计值，但大多数项目仍低于这一阈值。最后，人机协作以单一人类监督模式为主，即一个开发者审查和/或修改智能体的贡献，而多人协作模式并不常见。这些发现为开源项目如何围绕智能编码工具组织人类监督提供了早期实证证据，并表明智能体生成贡献的成功整合不仅取决于智能体能力的进步，还取决于管理其使用的人类和组织过程。由于本研究捕捉了智能体采纳的早期快照，未来工作应继续追踪采纳模式如何随时间演变。

Original abstract: Agentic coding tools are increasingly capable of generating and submitting pull requests (PRs) to software projects, introducing new forms of human-agent collaboration in software development. While prior studies have examined PR-level outcomes of agent-generated contributions, less is known about how agentic coding tools are adopted and managed at the project level. In this paper, we analyze 25,264 agentic PRs from 2,361 popular GitHub repositories to investigate (1) the adoption of agentic coding tools, (2) project-level agentic PR productivity, and (3) human-agent collaboration patterns. Our results show that the median repository generates only one to two agentic PRs during a three-month period, indicating that intensive adoption remains concentrated in a small subset of projects. At the same time, small projects (1-5 contributors) exhibit higher participation ratios and average levels of agentic PR activity than medium-sized and large projects. We also observe substantial variation in project-level agentic PR productivity. While a small number of projects exceed an industry-reported estimate of 36 PRs per participant during the three-month observation period, most projects remain below this threshold. Finally, human-agent collaboration is dominated by a single-human oversight model, in which one developer reviews and/or modifies the agent's contributions, while multi-human collaboration patterns remain uncommon. These findings provide early empirical evidence on how open-source projects organize human oversight around agentic coding tools and suggest that successful integration of agent-generated contributions depends not only on advances in agent capabilities but also on the human and organizational processes that govern their use. Because this study captures an early snapshot of agent adoption, future work should continue to track how adoption patterns evolve over time.

</details>


### [15] [VisualRepair: Dynamic Tool Calling and Region Focusing for Visual Software Issue Repair](https://arxiv.org/abs/2607.14075)
*Jingyu Xiao,Zhongyi Zhang,Haoran Hou,Yuxuan Wan,Yuan Jiang,Yintong Huo,Michael R. Lyu*

Main category: cs.SE

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: VisualRepair通过图像类型感知工具调用和动态测试时区域聚焦，利用多模态大语言模型有效修复软件问题，在SWE-bench Multimodal上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有程序修复方法难以处理多模态错误报告中异构视觉附件和无关区域干扰的问题。

Method: 提出VisualRepair框架，包含图像类型感知工具调用（ITTC）和动态测试时区域聚焦（DTRF）两个模块。

Result: 在SWE-bench Multimodal基准上，VisualRepair测试集解决196个实例，开发集解决25个实例，分别超过最佳基线10和11个实例。

Conclusion: VisualRepair有效利用类型感知视觉理解和区域聚焦定位，显著提升了自动化可视软件问题修复的性能。

中文摘要：自动程序修复（APR）随着大型语言模型（LLM）的出现取得了显著进展。然而，由于现代软件系统越来越多地暴露丰富的图形用户界面，在多模态场景中有效利用错误截图中的视觉信息对于理解错误并生成准确修复变得至关重要。真实世界的问题报告通常包含异构的视觉附件，包括用户界面截图、IDE快照、GIF和以文本为中心的图像，每种都带有独特的视觉模式和领域特定语义，给多模态大语言模型（MLLM）带来了巨大的感知需求。此外，错误截图通常包含大片无信息且与错误无关的区域，分散模型的注意力并限制补丁多样性。为了解决这些挑战，我们提出了VisualRepair，一个基于MLLM的可视化软件问题修复框架，包含两个核心模块：图像类型感知工具调用（ITTC），它对输入图像进行分类并动态调用定制的工具调用链以进行稳健的视觉解释；以及动态测试时区域聚焦（DTRF），它定位多个与错误相关的候选区域，并通过自适应缩放策略进行细化，以改进故障定位并促进多样化补丁生成。在SWE-bench Multimodal基准上的大量实验表明，VisualRepair始终优于最先进的方法。在测试集和开发集上，VisualRepair分别解决了196和25个实例，超过最佳基线10和11个实例。这些结果突显了类型感知视觉理解和区域聚焦定位在自动化可视软件问题修复中的有效性。

Original abstract: Automated Program Repair (APR) has witnessed significant progress with the advent of Large Language Models (LLMs). However, as modern software systems increasingly expose rich graphical user interfaces, effectively leveraging visual information from bug screenshots has become essential for understanding bugs and generating accurate fixes in multimodal scenarios. Real-world issue reports frequently contain heterogeneous visual attachments including UI screenshots, IDE snapshots, GIFs, and text-centric images, each with distinct visual patterns and domain-specific semantics that impose substantial perceptual demands on MLLMs. Furthermore, bug screenshots often contain large expanses of uninformative and bug-irrelevant regions, distracting the model's attention and limiting patch diversity. To address these challenges, we propose VisualRepair, an MLLM-based framework for visual software issue repair comprising two core modules: Image Type-aware Tool Calling (ITTC), which classifies input images and dynamically invokes a tailored tool-calling chain for robust visual interpretation, and Dynamic Test-time Region Focusing (DTRF), which grounds multiple bug-related region candidates and refines them via an adaptive zoom-in and zoom-out strategy to improve fault localization and promote diverse patch generation. Extensive experiments on the SWE-bench Multimodal benchmark demonstrate that VisualRepair consistently outperforms state-of-the-art approaches. VisualRepair resolves 196 and 25 instances on the test and dev sets, respectively, surpassing the best baseline by 10 and 11 instances. These results highlight the effectiveness of type-aware visual understanding and region-focused localization for automated visual software issue repair.

</details>


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [16] [SPINE: Bridging the Cyber-Physical Gap with Agentic AI](https://arxiv.org/abs/2607.13049)
*Minkyu Ham,Dongho Kim,Chan Lee,Jiayi Wang,Min Jun Kim,Yixi Zhang,Guo Ye,Jihai Zhao,Soyeon Park,Han Liu*

Main category: cs.AI

Selection reason: 多 agent 机器人调试框架，可迁移到编码 agent

TL;DR: SPINE是一个代理框架，通过多代理工作流自动化双臂机器人的调试和部署，减少专家依赖并提高成功率。


<details>
  <summary>Details</summary>
Motivation: 基础模型智能部署到物理机器人需要繁琐的专家校准，成为可扩展具身AI的主要瓶颈。

Method: SPINE框架包含两个编排的多代理工作流：配置文件构建器创建机器人特定上下文，调试器通过诊断-修复-验证循环自动化调试直至遥操作正常。

Result: 在DOBOT X-Trainer上，机器人新手使用SPINE成功率从75%提升至100%，平均遥操作时间从16分45秒降至13分47秒；在AgileX PiPER上解决所有10个植入bug（专家基线解决9/10），时间相近。

Conclusion: SPINE能跨双臂平台转移，减少对专家校准的依赖，推动具身AI向可扩展现实部署迈进。

中文摘要：基础模型赋予了机器人复杂的决策能力，但将这种智能部署到物理平台上仍然需要繁琐、专家驱动的校准。这种部署差距，即机器人的脊髓，仍然是可扩展具身智能的主要瓶颈。因此，我们提出SPINE（可扩展物理集成与智能代理）：一个代理框架，用于系统性地调试和部署双臂机器人，且只需极少的机器人专业知识。SPINE的装备包括两个编排的多代理工作流：一个配置文件构建器，用于创建机器人特定的上下文；以及一个调试器，它循环进行诊断、修复和验证，直到遥操作正常工作。在七个DOBOT X-Trainer调试场景中，一个使用SPINE的机器人新手表现优于使用Claude Code（相同参考资料但无SPINE结构化工作流）的人类操作员，将操作成功率从75%提高到100%，并将平均遥操作时间从16分45秒减少到13分47秒。在AgileX PiPER（一种不同的ROS/CAN双臂手臂）上，SPINE解决了所有10个植入的bug，而专家基线解决了9/10，且花费几乎相同的时间。这些结果表明，SPINE可以跨双臂平台转移，减少对专家校准的依赖，并使具身智能更接近可扩展的现实世界部署。

Original abstract: Foundation models have given robots a sophisticated brain for complex decision-making, yet deploying that intelligence into a physical platform still demands tedious, expert-driven calibration. This deployment gap, the robot's spinal cord, remains a primary bottleneck to scalable Embodied AI. Hence, we propose SPINE (Scalable Physical Integration with ageNtic Expertise): an agentic framework for systematically debugging and deploying bimanual robots with minimal robotics expertise. SPINE's harness comprises two orchestrated multi-agent workflows: a profile builder that creates robot-specific context, and a debugger that cycles through diagnosis, repair, and validation until teleoperation works. Across seven DOBOT X-Trainer debugging scenarios, a robotics novice using SPINE outperformed human operators using Claude Code with the same reference materials, but without SPINE's structured workflow, improving operationalization success from 75% to 100% and reducing mean time-to-teleoperation from 16 min 45 s to 13 min 47 s. On AgileX PiPER, a distinct ROS/CAN bimanual arm, SPINE resolved all 10 implanted bugs, versus 9 out of 10 for the expert baseline, in nearly the same amount of time. Together, these results show that SPINE can transfer across bimanual platforms, reduce dependence on expert calibration, and move embodied AI closer to scalable real-world deployment.

</details>


### [17] [Interventional Grounding Audits: Black-Box Premise-Dependency Tests for LLM Chain-of-Thought via Predicate Substitution](https://arxiv.org/abs/2607.13069)
*Hironao Nakamura*

Main category: cs.AI

Selection reason: LLM 推理审计方法，可作 agent 评估参考

TL;DR: 提出干预性基础审计方法，通过替换前提谓词测试推理步骤对前提的依赖，在ProntoQA上以GPT-4o达到F1=0.806，显著优于基线，并发现大量“正确答案，错误推理”案例。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的思维链推理可能在逻辑上看似合理，但实际上并不真正依赖于所陈述的前提，现有方法难以检测这种“伪因果”推理。

Method: 通过替换单个前提中的目标谓词为新符号，重新运行模型并检查每个推理步骤的规范化结论是否变化，从而在步骤级别测试前提依赖性。

Result: 在ProntoQA上使用GPT-4o，检测证明树依赖关系的F1=0.806（谓词决定性依赖F1=0.885，召回率100%），显著优于自一致性基线（F1=0.343）；66%的正确解决案例存在至少一个对直接证明树依赖不敏感的步骤。

Conclusion: 干预性基础审计能有效检测思维链推理中的前提依赖缺陷，揭示“正确答案，错误推理”现象，但需注意其适用范围限于形式化可解析基准。

中文摘要：大型语言模型产生的思维链推理在逻辑上看似合理，但其推理过程可能并不真正依赖于其所陈述的前提。我们引入了干预性基础审计，这是一种黑盒的、步骤级别的前提依赖性测试方法：我们通过将单个前提的目标谓词替换为一个新符号来进行干预，重新运行模型，并检查每个推理步骤的规范化结论（典型谓词形式）是否发生变化。我们在ProntoQA（一个具有已知步骤级前提依赖关系的合成多跳演绎推理基准）上进行了评估。将我们的方法应用于50个ProntoQA问题（使用GPT-4o），在检测证明树依赖关系时达到了F1=0.806（在谓词决定性依赖关系上F1=0.885；召回率=100%），显著优于自一致性基线（F1=0.343；95%自助法置信区间不重叠）。我们进一步发现，在正确解决的问题中，有66%的问题包含至少一个对齐步骤，该步骤在一致性替换下对直接证明树依赖不敏感——所有这些都涉及实体引入前提，这是一致性替换评估器的一个已知盲点——这是一种被动方法无法发现的“正确答案，错误推理”信号。所有审计证书、原始输出和复现脚本均可在公共GitHub存储库中获得，并且我们讨论了超出形式化可解析基准的范围限制。

Original abstract: Large language models produce chain-of-thought (CoT) reasoning that appears logically sound yet may not genuinely depend on its stated premises. We introduce interventional grounding audits, a black-box, step-level test of premise dependency: we intervene on a single premise by substituting its target predicate with a fresh symbol, re-run the model, and check whether each reasoning step's normalized conclusion (canonical predicate form) changes. We evaluate on ProntoQA, a synthetic multi-hop deductive reasoning benchmark with gold proof trees, where step-level premise dependencies are known. Applied to 50 ProntoQA problems with GPT-4o, our method achieves F1 = 0.806 on detecting proof-tree dependencies (F1 = 0.885 on predicate-determining dependencies; Recall = 100%), significantly outperforming a self-consistency baseline (F1 = 0.343; 95% bootstrap CIs non-overlapping). We further identify that 66% of correctly-solved problems contain at least one aligned step insensitive to a direct proof-tree dependency under consistent substitution -- all involving entity-introduction premises, a documented blind spot of the consistent-substitution evaluator -- a "right answer, wrong reasoning" signal invisible to passive methods. All audit certificates, raw outputs, and reproduction scripts are available in a public GitHub repository, and we discuss scope limits beyond formal, parsable benchmarks.

</details>


### [18] [Self-Improvements in Modern Agentic Systems: A Survey](https://arxiv.org/abs/2607.13104)
*Zhe Ren,Yimeng Chen,Dandan Guo,Guowei Rong,Tonghui Li,R. B. Xiong,Qingfeng Lan,Wenyi Wang,Li Nanbo,Yibo Yang,Mingchen Zhuge,Jürgen Schmidhuber*

Main category: cs.AI

Selection reason: 自改进 agent 综述，直接相关用户研究

TL;DR: 本综述提出一个系统级框架，将自我改进代理形式化为通过自诱导更新操作符改进模型参数或支架组件的自适应系统。


<details>
  <summary>Details</summary>
Motivation: 自我改进代理正从研究原型转向实际部署，需要统一视角来理解其可控进化机制并指导未来发展。

Method: 我们构建了一个框架，将代理表示为基模型与操作支架的配置，并将自我改进定义为自诱导更新操作符。通过按更新目标和驱动信号分类现有工作，系统化梳理了该领域。

Result: 该框架揭示了不同更新策略（参数更新与支架更新）和信号类型（如内部反馈、外部监督）的优缺点，并回顾了应用与评估方法，指出了开放问题。

Conclusion: 自我改进代理具有从经验中自主积累能力的潜力，本框架为未来研究提供了清晰的分类和比较基础，并指出了需要进一步探索的方向。

中文摘要：自我改进的自主代理正从研究原型转向部署系统。主要目标是在最少的甚至没有人类输入的情况下，通过经验实现可控进化或适应。本综述将现代自我改进代理视为将经验转化为累积能力增益的适应性系统。我们提供了一个系统级框架，将现代代理表示为耦合基础模型与提示、记忆、工具和控制逻辑的操作支架的配置。在该框架内，自我改进被形式化为一个自诱导的更新操作符，该操作符获取并提交对模型参数或支架组件的更新。我们按更新目标和驱动变化的信号组织先前的工作，然后回顾应用并讨论评估，最后以开放问题和未来方向作为结束。为了方便，我们在https://github.com/selfimproving-agent/awesome-Self-Improving-Agents上跟踪技术更新。

Original abstract: Self-improving autonomous agents are moving from research prototypes to deployed systems. The primary goal is controllable evolution, or adaptation, from experience with minimal or even no human input. This survey frames modern self-improving agents as adaptive systems that convert experience into accumulated capability gains. We offer a system-level framework that represents a modern agent as a configuration coupling a foundation model with an operational scaffold of prompts, memory, tools, and control logic. Within this framework, self-improvement is formalized as a self-induced update operator that obtains and commits updates to model parameters or scaffold components. We organize prior work by update target and by the signals that drive change, then review applications and discuss evaluation, before closing with open problems and future directions. For convenience, we track technical updates on https://github.com/selfimproving-agent/awesome-Self-Improving-Agents.

</details>


### [19] [Improving Molecular Property Prediction in Small Language Models Using Graph-based Tools](https://arxiv.org/abs/2607.13115)
*Konstantinos Bougiatiotis,Dimitrios Kelesis,Georgios Paliouras*

Main category: cs.AI

Selection reason: agent 工具使用框架，可迁移至编码场景

TL;DR: 提出上下文增强提示框架，利用GNN提供图信息提升SLM分子性质预测，准确率大幅提升但仍逊于专用GNN。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在分子性质预测中存在结构盲点，仅靠SMILES序列无法捕获图拓扑信息，需要融入图结构知识。

Method: 提出模块化上下文增强提示框架，在推理时调用GNN专家模型提供预测提示和置信度，并利用另一GNN提取解释性子图（子图SMILES及说明）作为提示上下文。

Result: 在MUTAG和Tox21上，相比仅SMILES提示，图增强提示使准确率相对提升25%至74%，且通过边删除实验验证了提取基序的功能相关性。

Conclusion: 图上下文显著增强了SLM的预测能力，但与专用GNN仍有差距，表明文本推理在分子结构理解上既有价值也有局限。

中文摘要：小语言模型（SLMs）在从SMILES字符串进行零样本分子性质预测方面显示出潜力，但由于序列表示未能充分指定关键的图拓扑线索，它们常常受到结构盲点的影响。我们提出了一种模块化的上下文增强提示框架，该框架在推理时实现代理工具使用：训练好的GNN专家模型提供带有置信度的预测提示，GNN提取实例特定的解释性子图（例如，子图SMILES和附带解释段落）。我们在MUTAG和Tox21上评估了三种常用的SLM，采用五种提示配置，范围从仅SMILES到使用所有可用工具。在两个数据集上，用图衍生上下文丰富提示带来了显著的准确率提升，相对改进通常超过25%，在Tox21上高达74%。我们进一步通过基于必要性的边删除干预验证了提取基序的功能相关性。尽管观察到了改进，但与专门的GNN模型之间仍存在持续差距，这突显了文本条件推理在分子结构方面的价值和局限性。

Original abstract: Small language models (SLMs) have shown promise for zero-shot molecular property prediction from SMILES strings, yet they often suffer from structural blindness because sequence representations under-specify key graph-topological cues. We propose a modular Context-Augmented Prompting framework that enables agentic tool use at inference time: a trained GNN expert model provides a predictive hint with confidence, and a GNN extracts an instance-specific explanatory subgraph (e.g., a subgraph SMILES and an accompanying explanatory paragraph). We evaluate three commonly used SLMs on MUTAG and Tox21 under five prompting configurations ranging from SMILES-only to using all available tools at hand. Across two datasets, enriching prompts with graph-derived context yields substantial accuracy gains, often exceeding 25% relative improvement and up to 74% on Tox21. We further validate the functional relevance of the extracted motifs via a necessity-based edge-drop intervention. Despite the observed gains, a persistent gap remains to specialized GNN models, highlighting both the value and limits of text-conditioned reasoning for molecular structure.

</details>


### [20] [Oracle Agent Memory as an Enterprise Memory Substrate for Long-Horizon AI Agents](https://arxiv.org/abs/2607.13157)
*Richmond Alake,Cesare Bernardis,Paul Cayet,Luca Engel,Damien Hilloulin,Sungpack Hong,Allen Hosler,Nickolas Kavantzas,Ingo Kossyk,Son Le,Rhicheek Patra,Kartik Talamadupula,Valentin Venzin*

Main category: cs.AI

Selection reason: agent 记忆系统，通用方法可迁移

TL;DR: 提出Oracle Agent Memory作为数据库原生记忆系统，通过分层架构和生命周期管理解决长时程智能体的记忆问题，在LongMemEval上达到93.8%准确率且节省10.7倍令牌。


<details>
  <summary>Details</summary>
Motivation: 现有长时程智能体部署需要保留任务状态、用户偏好和程序性知识，但缺乏系统化的记忆管理，尤其是超过简单文档检索的能力，需要确定持久状态、范围控制、低延迟检索和更新机制。

Method: 基于Oracle数据库构建数据库原生记忆基板，采用三层组织：记忆生命周期（摄取、提取、整合、检索、总结、修订/移除）、分层架构（主动记忆核心与被动存储接口分离，支持跨用户/智能体/线程范围控制），以及评估方法（结合下游任务准确率和记忆中心指标如证据检索、召回率、延迟、令牌使用）。

Result: 在LongMemEval上达到93.8%准确率，与平面历史基线相比使用约10.7倍更少的令牌，并在可用时与外部基线比较。

Conclusion: Oracle Agent Memory提供了一种有效的数据库原生记忆方案，通过系统化的生命周期和分层架构在保持高准确率的同时大幅降低令牌消耗，适用于长时程智能体部署。

中文摘要：智能体记忆是长时程智能体的系统问题。实际部署需要在扩展对话中保留任务状态，跨会话恢复用户特定的事实和偏好，以及从先前结果中积累程序性知识。这些需求超越了文档检索：记忆层必须确定哪些交互成为持久状态、该状态如何划定范围、如何在延迟约束下检索，以及如何随时间修订或移除。本报告将Oracle Agent Memory作为基于Oracle数据库的数据库原生记忆基板进行研究。三个主题组织讨论：记忆作为生命周期，涵盖摄取、提取、整合、检索、总结和修订或移除；分层架构，将主动记忆核心与被动记忆存储接口分离，并跨用户、智能体和线程进行显式范围控制；评估方法，其中下游任务准确性辅以以记忆为中心的度量，如证据检索、召回率、延迟和估计的令牌使用。报告总结了LongMemEval结果，达到93.8%准确率，将Oracle Agent Memory与平面历史基线进行比较，使用约10.7倍更少的令牌，并在可用时与已发布或报告的外部基线进行比较，最后以面向实现的附录材料结束，涵盖设置、线程生命周期和搜索语义。

Original abstract: Agent memory is a systems problem for long-horizon agents. Practical deployments require retention of task state across extended conversations, recovery of user-specific facts and preferences across sessions, and accumulation of procedural knowledge from prior outcomes. These requirements extend beyond document retrieval: a memory layer must determine which interactions become durable state, how that state is scoped, how it is retrieved under latency constraints, and how it is revised or removed over time. This report studies Oracle Agent Memory as a database-native memory substrate built on Oracle Database. Three themes organize the discussion: memory as a lifecycle spanning ingestion, extraction, consolidation, retrieval, summarization, and revision or removal; a layered architecture that separates an active memory core from a passive memory-store interface with explicit scope control across users, agents, and threads; and evaluation methodology in which downstream task accuracy is complemented by memory-centric measures such as evidence retrieval, recall, latency, and estimated token use. The report summarizes LongMemEval results, reaching 93.8% accuracy, compares Oracle Agent Memory against flat-history baselines, using about 10.7x fewer tokens, and published or reported external baselines where available, and closes with implementation-oriented appendix material covering setup, thread lifecycle, and search semantics.

</details>


### [21] [Learning Safe Agent Behaviour from Human Preferences and Justifications via World Models](https://arxiv.org/abs/2607.13172)
*Ilias Kazantzidis,Timothy J. Norman,Yali Du,Christopher T. Freeman*

Main category: cs.AI

Selection reason: 基于人类偏好的安全 agent 训练，方法可迁移

TL;DR: 提出DROPJ方法，利用人在模拟器中提供偏好及理由来训练奖励模型，结合世界模型进行安全部署，实验证明能降低计算成本并提升安全性和性能。


<details>
  <summary>Details</summary>
Motivation: 在环境动态未知且无合适奖励函数的安全关键场景中，传统强化学习不实用，需要利用人类输入来安全地训练和部署策略。

Method: 先学习世界模型，让用户在模拟器中生成信息轨迹；采样轨迹片段对，让用户提供偏好及理由；训练奖励模型，结合世界模型用模型预测控制部署智能体。

Result: 生成信息轨迹显著降低训练计算成本，提高部署性能；偏好反馈优于其他类型反馈；安全理由能增强安全性或优先用户指定的安全方面。

Conclusion: DROPJ方法通过人类偏好及理由实现了安全训练和部署，在计算成本和性能上优于其他策略，并能通过安全理由提升安全性。

中文摘要：我们解决了在环境动力学未知且没有合适的奖励函数可用的情况下，安全地训练智能体策略并部署良好且安全策略的问题。在安全关键环境的背景下，我们考虑传统的强化学习不切实际，并求助于人类输入的资源。我们引入了DROPJ，一种以人为本的方法，用于安全训练和部署。我们首先从先前的真实世界轨迹数据集中学习一个世界模型（一个学习的模拟器）。然后，人类在这个学习的模拟器中玩游戏，提取若干信息丰富的模拟轨迹。从中，我们采样模拟轨迹片段对，并让人类选择对这些片段的偏好，以及他们选择的理由（正当理由）。然后，我们根据这些有理由的偏好训练一个奖励模型，并将其与世界模型一起使用，通过模型预测控制直接部署智能体。通过运行真实用户实验，我们发现，与其它策略相比，从用户生成信息丰富的模拟轨迹显著降低了训练期间的计算成本，并且还可以提高部署期间的性能。在学习模拟器中训练的背景下，我们表明，使用偏好而非其他类型的反馈显著提高了部署期间的性能。我们进一步证明，伴随偏好的安全理由可以显著增强安全性，或优先考虑用户指定的与它们相关的安全方面。

Original abstract: We address the problem of safely training an agent policy and deploying a good and safe policy, in settings where the environment dynamics are unknown and no suitable reward function is available. In the context of safety-critical environments, we consider traditional reinforcement learning impractical and resort to the resource of human input. We introduce DROPJ, a human-centred method for both safe training and deployment. We first learn a world model (a learned simulator) from a dataset of prior real-world trajectories. A human then plays the game in this learned simulator to extract several informative simulated trajectories. From these, we sample pairs of simulated trajectory segments and elicit from a human their preference over these segments, as well as a reason (justification) for their choice. We then train a reward model from these justified preferences and use it, together with the world model, to directly deploy the agent using model predictive control. Running real-user experiments, we find that generating informative simulated trajectories from a user significantly reduces the computational cost during training compared to other strategies, and can also improve the performance during deployment. In the context of training within a learned simulator, we show that the use of preferences rather than other types of feedback substantially improves the performance during deployment. We further demonstrate that safety justifications accompanying preferences can significantly enhance safety or prioritise user-prescribed aspects of safety associated with them during deployment.

</details>


### [22] [Networked Intelligence: Active Shared Context Graphs for Human-AI Team Science](https://arxiv.org/abs/2607.13220)
*Sutanay Choudhury,Jeffrey J. Czajka,Lummy M. O. Monteiro,Erin Bredeweg,Jason McDermott,Katherine Wolf,Alex Beliaev,Josh Elmore,Paul Piehowski,Kylee Tate,Yuqian Gao,Aivett Bilbao,Kelly Stratton,Scott Baker,Jaydeep P. Bardhan,Kristin Burnum Johnson,Chris Oehmen,Robert Rallo*

Main category: cs.AI

Selection reason: 多用户 agent 协作系统，可迁移

TL;DR: Mycelium是一个主动共享工作空间，通过自动连接研究人员和AI智能体并路由共享上下文，促进团队科学发现。


<details>
  <summary>Details</summary>
Motivation: 现有AI-for-science系统局限于单一推理过程，但科学问题需多背景团队协作，因此需建立连接人机与AI的网络智能以共享和传递知识。

Method: 构建Mycelium系统，作为多用户共同科学家的主动共享工作空间，捕获并追踪团队产出，将信息路由至相关决策者。

Result: 在生物多组学活动中，路由共享上下文将局部分析转化为跨专家机制约束，并最终形成实验设计。

Conclusion: 网络智能可建模为分布式科学上下文上的稀疏条件计算，当上下文不可合并时，网络具有不可约简优势。

中文摘要：大多数AI for science系统专注于通过更好的模型、更大的上下文窗口、长周期自主执行或与一位主要用户合作的数字共同科学家来扩展单一推理过程。然而，具有挑战性的科学问题很少由单个推理者解决。它们是由团队解决的，团队成员带来不同的先验知识、实验背景、隐性知识和领域训练直觉。因此，开放问题不仅是如何扩展模型，而是如何培养网络智能：扩展人类与AI系统之间的联系，使得在一个上下文中产生的结果或假设能够到达另一个可以对其采取行动的人、智能体、仪器或机器人。我们介绍了Mycelium，一个主动共享工作空间，自动连接研究人员和AI智能体，作为一个多用户共同科学家。当人类用户和智能体工作时，系统捕获重要的观察结果和假设，追踪它们如何与团队不断演化的模型相关，并将它们路由到下一个决策可能受益的人或智能体。我们在其首次实证测试中评估了Mycelium，一个生物多组学活动，其中路由共享上下文将局部分析发现转化为跨专家的机制约束，最终转化为实验设计。我们还为网络智能提供了一个计算解释，作为分布式科学上下文上的稀疏条件计算。这个解释区分了何时独立的智能体能够匹配网络，以及何时独立的专业知识和不可合并的上下文使网络不可约简。

Original abstract: Most AI-for-science systems focus on scaling a single reasoning process through better models, larger context windows, long-horizon agentic execution, or digital co-scientists working with one principal user. However, challenging scientific problems are rarely solved by one reasoner alone. They are solved by teams whose members bring different priors, experimental backgrounds, tacit knowledge, and domain-trained intuitions. The open problem is therefore not only how to scale models, but how to cultivate networked intelligence: scaling the connections between humans and AI systems so that a result or hypothesis produced in one context reaches another person, agent, instrument, or robot that can act on it. We introduce Mycelium, an active shared workspace that automatically connects researchers and AI agents as a multi-user co-scientist. As human users and agents work, the system captures important observations and hypotheses, tracks how they relate to the team's evolving model, and routes them to the person or agent whose next decision they can inform. We evaluate Mycelium in its first empirical test, a biological multi-omics campaign in which routed shared context turned a local analytical finding into a cross-expert mechanistic constraint and ultimately into an experimental design. We also give networked intelligence a computational account as sparse conditional computation over distributed scientific contexts. This account distinguishes when a scaled standalone agent can match the network from when independent expertise and non-mergeable contexts make the network irreducible.

</details>


### [23] [Harness Handbook: Making Evolving Agent Harnesses Readable,Navigable, and Editable](https://arxiv.org/abs/2607.13285)
*Ruhan Wang,Yucheng Shi,Zongxia Li,Zhongzhi Li,Yue Yu,Junyao Yang,Kishan Panaganti,Haitao Mi,Dongruo Zhou,Leoweiliang*

Main category: cs.AI

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出两种方法（Harness Handbook和BGPD）来自动定位AI代理鞍具代码中的行为，以提高修改效率。


<details>
  <summary>Details</summary>
Motivation: 当前鞍具代码修改困难，因为行为分散在代码中，手动定位效率低，成为演化瓶颈。

Method: 通过静态分析和LLM自动构建行为为中心的表示（Harness Handbook），并采用行为引导的渐进式披露（BGPD）指导代理定位。

Result: 在开源鞍具上，方法提高了行为定位和编辑计划质量，减少了规划令牌使用，尤其对分散、少执行和跨模块行为效果显著。

Conclusion: 演化复杂AI代理系统不仅需要生成编辑，还需要确定编辑位置，所提方法有效解决了行为定位问题。

中文摘要：现代AI代理的能力不仅取决于其基础模型，还取决于其“鞍具”（harness），后者负责构建提示、管理状态、调用工具并协调执行。随着模型、API、环境和需求的变化，鞍具必须不断修改。在进行此类修改之前，开发者或编码代理必须识别实现目标行为的所有代码位置。这很困难，因为生产环境中的鞍具规模大、耦合度高且行为分布广泛，而修改请求描述的是系统应该做什么，仓库则按文件和模块组织。代码搜索、仓库索引和长上下文处理简化了检查，但仍需手动恢复这种行为到代码的映射。因此，行为定位成为鞍具演化的核心瓶颈。我们引入了“鞍具手册”（Harness Handbook），这是一种从鞍具代码库中通过静态分析和LLM辅助结构自动合成的以行为为中心的表示，将每个行为链接到其对应源码。我们还引入了行为引导的渐进式披露（BGPD），它引导代理从高层行为到相关实现细节，并对照当前源码验证候选位置。在两个开源鞍具的多样化修改请求上，手册辅助规划提高了行为定位和编辑计划的质量，同时使用了更少的规划令牌，最大的收益体现在分散的站点、极少执行的路径和跨模块交互上。演化复杂的代理系统不仅依赖于生成编辑，还取决于确定这些编辑应该在何处进行。

Original abstract: The capability of a modern AI agent depends not only on its foundation model but also on its harness, which constructs prompts, manages state, invokes tools, and coordinates execution. As models, APIs, environments, and requirements evolve, the harness must be continually modified. Before such a change can be made, a developer or coding agent must identify all code locations that implement the target behavior. This is difficult because production harnesses are large, tightly coupled, and behaviorally distributed, while modification requests describe what the system should do and repositories are organized by files and modules. Code search, repository indexing, and long-context processing ease inspection, but still leave this behavior-to-code mapping to be recovered by hand. Behavior localization is therefore a central bottleneck in harness evolution. We introduce the Harness Handbook, a behavior-centric representation synthesized automatically from a harness codebase via static analysis and LLM-assisted structuring, linking each behavior to its corresponding source. We also introduce Behavior-Guided Progressive Disclosure (BGPD), which guides agents from high-level behaviors to relevant implementation details and verifies candidate locations against the current source. On diverse modification requests from two open-source harnesses, Handbook-Assisted planning improves behavior localization and edit-plan quality while using fewer planner tokens, with the largest gains on scattered sites, rarely executed paths, and cross-module interactions. Evolving complex agentic systems thus depends not only on generating edits, but also on determining where those edits should be made.

</details>


### [24] [Set-shifting Behavioral Test for Harnessed Agents](https://arxiv.org/abs/2607.13396)
*Ziwei Ye*

Main category: cs.AI

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 研究LLM智能体在工具可靠性发生隐藏变化时适应性的基准和评估，发现智能体形成固定惯例，集合框架影响路由动态。


<details>
  <summary>Details</summary>
Motivation: 探索LLM智能体在真实动态环境中的工具选择适应性，特别是当工具可靠性在会话中无声变化时，现有研究缺乏对此场景的系统评估。

Method: 构建冗余工具-技能库基准，采用分支计划模拟隐藏可靠性转移，并与无转移对照配对；使用集合转换准确性作为评估指标，测试开放权重LLM。

Result: 智能体在可靠性转移后迅速形成固定惯例，调用份额集中在少数离散值；不同模型表现出定性不同的失败模式；集合框架（竞争/互补）显著改变路由动态。

Conclusion: LLM智能体对工具可靠性变化适应不足，易陷入固定套路，且工具呈现方式影响其适应行为，提示需要改进智能体动态决策能力。

中文摘要：当可靠的智能体工具在持续会话中悄然发生变化时，LLM智能体的工具选择会发生什么？我们从认知心理学借用了集合转换来研究智能体适应隐藏可靠性转移的能力。我们的基准测试构建了具有冗余的工具-技能库，其中多个工具可以解决同一任务但隐藏的可靠性不同。在我们的评估框架中，一个分支计划在隐藏边界处转移可靠工具组，并将每个转移与一个无转移对照配对。我们发现，智能体默认在每个边界后的几个回合内形成一个小型重复惯例，每次可靠性转移后调用份额集中在几个离散值上。我们为每个智能体轨迹评分集合转换准确性：在每个转移后窗口中路由到目标工具组的联合概率。我们在一个开源的智能体框架中测试了开放权重的LLM，并发现相同惯例集合中存在定性不同的失败模式。我们还发现，集合框架（即工具集如何将替代方案呈现为竞争或互补）会改变路由动态。

Original abstract: What happens to an LLM agent's tool choice when the reliable tool silently changes within an ongoing session? We borrow set-shifting from cognitive psychology to study how well agents adapt to hidden reliability shifts. Our benchmark mounts tool-skill libraries with redundancies, where many tools solve the same task but differ in hidden reliability. In our evaluation framework, a branched schedule shifts the reliable tool group at hidden boundaries and pairs every shift with a no-shift control. We find that agents, by default, settle on a small recurring routine within a few turns of each boundary, with call shares concentrating on a few discrete values after each reliability shift. We score the set-shifting accuracy for each agent trajectory: the joint probability of routing to the target tool group in every post-shift window. We test open-weight LLMs in an open-source agentic harness and find qualitatively distinct failure modes across the same set of routines. We also find that set framing, how the toolset presents the alternatives as competing or complementary, shifts the routing dynamics.

</details>


### [25] [AgentCompass: A Unified Evaluation Infrastructure for Agent Capabilities](https://arxiv.org/abs/2607.13705)
*Zichen Ding,Jiaye Ge,Shufan Jiang,Kai Chen,Mo Li,Qingqiu Li,Zehao Li,Zonglin Li,Tiaohao Liang,Shudong Liu,Zerun Ma,Zixing Shang,Wenhui Tian,Zun Wang,Liwei Wu,Zhenyu Wu,Jun Xu,Bowen Yang,Dingbo Yuan,Qi Zhang,Songyang Zhang,Peiheng Zhou,Dongsheng Zhu*

Main category: cs.AI

Selection reason: cs.SE 论文按全量策略收录。

TL;DR: 提出了AgentCompass，一个开源、轻量、可扩展的LLM代理评估基础设施，通过解耦基准、测试平台和环境，支持灵活配置、容错异步运行时和轨迹分析，原生覆盖20+基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理评估流程高度碎片化、紧耦合，导致可重复性差和工程冗余，亟需统一的基础设施。

Method: 围绕Benchmark、Harness、Environment三个独立组件构建评估流程，支持灵活配置；采用容错异步运行时和轨迹分析工具。

Result: 原生支持20+基准测试，覆盖五个能力维度，实现可扩展和可重复的评估。

Conclusion: AgentCompass为社区提供了统一的评估基础设施，有助于推动LLM代理研究的标准化和进步。

中文摘要：随着大型语言模型（LLM）演变为自主代理，统一的评估基础设施需求变得至关重要。然而，当前的评估流程仍然高度碎片化且紧密耦合，阻碍了可重复性并导致冗余工程。为解决这一问题，我们提出了AgentCompass，一个用于评估基于LLM的代理的开源、轻量级且可扩展的基础设施。AgentCompass围绕三个独立组件（即基准、测试平台和环境）组织评估过程，从而实现了灵活配置，无需重新实现复杂的执行逻辑。此外，它具备容错异步运行时和全面的轨迹分析工具，能够透明地诊断如奖励黑客等细微的失败模式。AgentCompass原生支持五个能力维度上的20多个基准测试，为社区提供了一个可扩展且可重复的基础设施，以推进代理研究。

Original abstract: As Large Language Models (LLMs) evolve into autonomous agents, the need for unified evaluation infrastructure becomes critical. However, current evaluation pipelines remain highly fragmented and tightly coupled, hindering reproducibility and causing redundant engineering. To address this, we introduce AgentCompass, an open-source, lightweight, and extensible infrastructure for evaluating LLM-based agents. AgentCompass organizes the evaluation process around three independent components, namely Benchmark, Harness, and Environment, thereby enabling flexible configurations without requiring the reimplementation of complex execution logic. Furthermore, it features a fault-tolerant asynchronous runtime and comprehensive trajectory analysis tools to transparently diagnose nuanced failure modes like reward-hacking. Natively supporting over 20 benchmarks across five capability dimensions, AgentCompass provides the community with a scalable and reproducible infrastructure for advancing agent research.

</details>


### [26] [LAPO: Leave-One-Turn Attribution for Self-Generated Process Rewards in Multi-Turn Search Reasoning](https://arxiv.org/abs/2607.13501)
*Qiang Zhu,Jiajun Wu*

Main category: cs.AI

Selection reason: 搜索 agent 过程奖励方法，直接相关

TL;DR: LAPO是一种基于后向留一回合归因的自生成过程监督方法，通过答案似然增益估计回合贡献，无需额外奖励模型，在多个数据集上优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法仅依赖终端结果奖励，无法区分中间交互的有用性。

Method: LAPO通过替换每个搜索回合为[DELETE]并测量答案似然增益来估计贡献，并应用符号一致性门控。

Result: 在七个知识密集型问答数据集上，LAPO平均准确匹配得分0.326，超过IGPO 0.053。

Conclusion: 策略派生的回溯归因可提供有效的过程监督，后向归因和符号一致性门控互补。

中文摘要：用于多轮搜索推理的强化学习通常依赖于终端结果奖励，这无法区分有用、冗余和有害的中间交互。我们提出了LAPO，一种基于后向留一回合归因的自生成过程监督方法。对于每个搜索回合，LAPO用固定的[DELETE]占位符替换该回合及其检索观测值，并测量由此导致的当前策略对正确答案的均值对数似然的变化。这种答案似然增益估计了该回合的贡献，同时保留了所有下游交互，使得早期证据能够在完整的推理上下文中进行评估。LAPO进一步应用符号一致性门控，仅保留方向与其原始归因分数一致的标准化过程优势。该方法不需要额外的奖励模型、教师、验证器或LLM作为评判者。在七个知识密集型问答数据集上，使用局部检索，LAPO的平均准确匹配得分为0.326，比最强的步骤奖励基线IGPO高出0.053。消融实验显示后向归因和符号一致性门控具有互补优势，表明策略派生的回溯归因可以为多轮搜索代理提供有效的过程监督。

Original abstract: Reinforcement learning for multi-turn search reasoning typically relies on terminal outcome rewards, which cannot distinguish useful, redundant, and harmful intermediate interactions. We propose LAPO, a self-generated process-supervision method based on backward leave-one-turn attribution. For each search turn, LAPO replaces the turn and its retrieval observation with a fixed [DELETE] placeholder and measures the resulting change in the current policy's mean log-likelihood of the gold answer. This Answer-Likelihood Gain estimates the turn's contribution while preserving all downstream interactions, allowing early evidence to be evaluated in the complete reasoning context. LAPO further applies sign-consistency gating, retaining only normalized process advantages whose directions agree with their raw attribution scores. The method requires no additional reward model, teacher, verifier, or LLM-as-a-Judge. Across seven knowledge-intensive question-answering datasets with local retrieval, LAPO achieves an average exact-match score of 0.326, outperforming the strongest step-reward baseline, IGPO, by 0.053. Ablations show complementary benefits from backward attribution and sign-consistency gating, demonstrating that policy-derived retrospective attribution can provide effective process supervision for multi-turn search agents.

</details>


### [27] [How Far Can Root Cause Analysis Go on Real-World Telemetry Data?](https://arxiv.org/abs/2607.13548)
*Athira Gopal,Ashwanth Krishnan*

Main category: cs.AI

Selection reason: 多 agent 根因分析管道，可迁移至编码 agent

TL;DR: 提出结构化多智能体RCA管道，通过反向推理分析发现故障根因的主要瓶颈是模型推理能力而非数据缺失，并引入自动规则挖掘减少人工依赖。


<details>
  <summary>Details</summary>
Motivation: 现有经典和LLM方法在OpenRCA数据集上准确率低，需要更有效的根因分析方法。

Method: 构建结构化多智能体RCA管道，包含反向推理代理（分类推理差距与数据模糊性）和自动规则挖掘管道。

Result: 该方法在基准上显著优于现有基线，并揭示大多数故障证据存在但推理不足，模型推理能力和领域知识是主要约束。

Conclusion: 即使在完美证据提取下，推理性能仍有上限；需通过模型级别改进而非单纯工程优化来突破瓶颈。

中文摘要：在生产环境微服务故障中识别根本原因需要对涵盖指标、日志和追踪的大规模多模态遥测数据进行推理，这个问题已被证明对经典方法和基于LLM的方法都具有抵抗力。OpenRCA数据集体现了这些挑战：它是大规模、多模态的，并且缺乏详细的领域知识，在所有现有方法上始终产生较低的准确率。我们表明经典因果发现方法和现有的基于LLM的多智能体系统未能可靠地在该基准上识别根本原因，并提出了一种结构化多智能体RCA管道，其显著优于现有的基于LLM和经典的基线，支持领域知识和无知识两种操作模式。为了诊断故障起源，我们引入了一个反向推理代理，在给出正确答案的情况下，识别提取的异常中的哪些信号支持它，并确定第一阶段是否能够访问这些信号，将每个故障分类为推理差距（证据存在但未使用）或数据模糊性（证据确实缺失）。该分析表明，在绝大多数故障中，所需的证据都存在：瓶颈不是数据访问，而是代理正确推理的能力。我们进一步引入了一个自动规则挖掘管道，从反向推理报告中系统性地提取判别规则，减少对手动知识整理的依赖。在所有配置中，模型推理能力和领域知识是主要限制：更强的模型嵌入了更多的领域专业知识，而显式知识注入部分地弥补了这一差距。即使证据提取完美，推理性能仍然受到实际限制：仅凭脚手架工程和更好的数据管道无法弥补这一差距；进步需要在模型层面进行改进。

Original abstract: Identifying root causes in production microservice failures requires reasoning over large-scale, multimodal telemetry spanning metrics, logs, and traces, a problem that has proved resistant to both classical and LLM-based approaches. The OpenRCA dataset exemplifies these challenges: it is large-scale, multimodal, and lacks detailed domain knowledge, and yields consistently low accuracy across all existing methods. We show that classical causal discovery methods and existing LLM-based multi-agent systems fail to reliably identify root causes on this benchmark, and present a Structured Multi-Agent RCA pipeline that substantially outperforms existing LLM-based and classical baselines, supporting both domain-knowledge and knowledge-free operating modes. To diagnose where failures originate, we introduce a reverse reasoning agent that, given the correct answer, identifies which signals in the extracted anomalies support it and determines whether Stage~1 had access to those signals, classifying each failure as Reasoning Gap (evidence present but unused) or Data Ambiguity (evidence genuinely absent). This analysis reveals that the required evidence is present in the vast majority of failures: the bottleneck is not data access but the agent's ability to reason over it correctly. We further introduce an automated rule mining pipeline that systematically extracts discrimination rules from reverse reasoning reports, reducing reliance on manual knowledge curation. Across all configurations, model reasoning capability and domain knowledge are the primary constraints: stronger models embed more domain expertise, and explicit knowledge injection partially compensates for this gap. Reasoning performance remains practically bounded even when evidence extraction is perfect: scaffold engineering and better data pipelines alone cannot close this gap; progress requires improvements at the model level.

</details>


### [28] [Multi-Agent Collaborative Reasoning with Tool-Augmented Evidence for Urban Region Profiling](https://arxiv.org/abs/2607.13558)
*Xixuan Hao,Yutian Jiang,Jiabo Liu,Yihang Yang,Guangyin Jin,Song Gao,Yuxuan Liang*

Main category: cs.AI

Selection reason: 多 agent 协作推理框架，可迁移

TL;DR: 提出UrbanAgent，一种基于多智能体协作推理的框架，将城市区域概况描述从相关驱动转变为推理驱动，在碳排放、GDP和人口估计任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于相关性和跨模态一致性假设，使用静态流程，在异构或未见区域中鲁棒性不足，需要一种能显式处理跨模态不一致性和推理不确定性的方法。

Method: UrbanAgent为每种数据模态独立实例化智能体，通过结构化多智能体协作推理处理跨模态不一致性；并将指标预测扩展为主动证据获取和迭代推理的闭环，利用强化学习优化的工具增强外部知识检索来验证推断。

Result: 在全球城市数据集上，UrbanAgent在碳排放、GDP和人口估计任务中平均R²提升8.1%，且在未见城市场景中表现出强泛化能力。

Conclusion: UrbanAgent通过推理驱动和多智能体协作机制，有效解决了跨模态不一致性，显著提高了城市区域概况描述的准确性和鲁棒性。

中文摘要：城市区域概况描述是城市计算中的核心问题，支持人口估计、经济评估和环境监测等应用。现有方法通常将此任务建模为多模态表示学习，将异构城市数据（例如卫星图像、兴趣点、文本描述和3D建筑信息）融合到潜在嵌入中进行预测。然而，这些方法本质上由相关性驱动，假设跨模态一致性，并依赖于静态流程，这限制了它们在异构或未见城市区域中的鲁棒性。我们提出UrbanAgent，一种智能体框架，将城市区域概况描述重新定义为推理驱动的推断问题。UrbanAgent为每种数据模态实例化一个独立的智能体，并执行结构化的多智能体协作推理，以明确处理跨模态不一致性，而不是将其吸收到单一表示中。此外，UrbanAgent将指标预测扩展为主动证据获取和迭代推理的闭环过程，使智能体能够通过经由强化学习优化的工具增强外部知识检索来验证不确定的推断。在全球城市数据集上针对碳排放、GDP和人口估计的广泛实验表明，UrbanAgent持续优于现有基线，R²平均提升8.1%，并在未见过城市的环境中展现出强大的泛化性能。

Original abstract: Urban region profiling constitutes a core problem in urban computing, supporting applications such as population estimation, economic assessment, and environmental monitoring. Existing methods typically formulate this task as multimodal representation learning, fusing heterogeneous urban data, e.g., satellite imagery, points of interest, textual descriptions, and 3D building information, into latent embeddings for prediction. However, these approaches are largely correlation-driven, assume cross-modal consistency, and rely on static pipelines, which limit their robustness in heterogeneous or unseen urban regions. We propose UrbanAgent, an agentic framework that reframes urban region profiling as a reasoning-driven inference problem. UrbanAgent instantiates an independent agent for each data modality and performs structured multi-agent collaborative reasoning to explicitly address cross-modal inconsistencies rather than absorbing them into a single representation. In addition, UrbanAgent extends indicator prediction as a closed-loop process of active evidence acquisition and iterative reasoning, enabling agents to verify uncertain inferences through tool-augmented retrieval of external knowledge optimized via reinforcement learning. Extensive experiments on global urban datasets for Carbon emissions, GDP, and Population estimation show that UrbanAgent consistently outperforms existing baselines, achieving an average improvement of 8.1% in R2, and exhibiting strong generalization performance in unseen-city settings.

</details>


### [29] [SAFETY SENTRY: Context-Aware Human Intervention via EXECUTE-ASK-REFUSE Routing](https://arxiv.org/abs/2607.13594)
*Tianyu Chen,Chujia Hu,Wenjie Wang*

Main category: cs.AI

Selection reason: agent 安全路由方法，通用 agent 安全控制

TL;DR: 提出Safety Sentry，通过三向路由决策（执行/询问/拒绝）实现细粒度的安全控制，优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有二元守卫模型混淆了动作本身有害性与上下文适宜性，且操作粒度粗导致频繁中断，损害用户自主性。

Method: 将安全决策重构为每实例的三向路由（EXECUTE, ASK, REFUSE），使用轻量级模型Safety Sentry，其推理仅需一次解码调用，并通过解码阈值调整风险容忍度。

Result: Safety Sentry在整体准确性和安全相关召回率上优于多种开源和闭源基线，同时有效控制两类错误率。

Conclusion: Safety Sentry提供了一种更细粒度、可灵活调整的安全防护机制，提升了代理系统的安全性和可用性。

中文摘要：LLM代理通过工具调用在真实世界环境中行动，一个错误的判断动作可能导致不可逆的伤害。标准的防护措施是一个守卫模型，它将每个提议的动作标记为安全或不安全，但这种二元视图混淆了两个不同的决定：动作本身是否有害，以及它是否适合用户的上下文。它还在动作类别而非单个实例的粒度上运行，导致常规中断，侵蚀自主性，并训练用户对最关键的警报视而不见。我们将问题重新定义为每个实例的三向路由决策，即{执行，询问，拒绝}，并用Safety Sentry实现，这是一个轻量级守卫模型，其推理归结为单个解码调用。单个解码时间阈值允许一个固定的检查点跨不同风险容忍度的部署重新定位，而无需重新训练。Safety Sentry在整体准确性和与安全相关的召回率上优于广泛的开源和前沿闭源基线，同时控制两个方向的错误率。

Original abstract: LLM agents act on real-world environments through tool calls, and a single misjudged action can cause irreversible harm. The standard safeguard is a guard model that labels each proposed action as safe or unsafe, but this binary view conflates two distinct decisions: whether the action is harmful in itself, and whether it is appropriate given the user's context. It also operates at the granularity of action categories rather than individual instances, producing routine interruptions that erode autonomy and train users to wave through the most consequential alerts. We reframe the problem as a per-instance three-way routing decision over {EXECUTE, ASK, REFUSE} and instantiate it with Safety Sentry, a lightweight guard model whose inference reduces to a single decoding call. A single decoding-time threshold lets one fixed checkpoint be re-positioned across deployments of differing risk tolerance without retraining. Safety Sentry outperforms a broad set of open-weight and frontier closed-source baselines on overall accuracy and safety-related recall, while controlling both directional error rates simultaneously.

</details>


### [30] [Automatic Ordinary Differential Equations Discovery For Biological Systems Using Large Language Model Powered Agentic System](https://arxiv.org/abs/2607.13608)
*David Krongauz,Arad Zulti,Eran Segal,Teddy Lazebnik*

Main category: cs.AI

Selection reason: LLM agent 科学发现框架，可迁移至优化

TL;DR: 提出基于LLM和符号回归的MEDA框架，用于自动发现生物系统的常微分方程模型，在多项任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方程发现方法多聚焦于狭窄基准或端到端流水线，生物系统未被充分探索；因此需要一种能够结合知识引导和机制约束的自动化建模框架。

Method: MEDA集成LLM和符号回归，通过检索背景知识、定义变量、生成机制约束、提出并拟合候选ODE，对生物动态系统进行模型发现。

Result: 在规范模型检索、外推和开放发现任务中，MEDA正确恢复状态变量并实现强结构恢复，生成生物学可信模型。消融分析表明知识引导和机制约束是关键组件。

Conclusion: MEDA展示了LLM和符号回归结合在自动科学发现中的潜力，但数值拟合可能保留生物学不正确的方程，需结合知识约束。

中文摘要：自动科学发现长期以来一直是计算学者的目标——一台能够自行发现自然奥秘的机器，将计算系统从数据拟合工具推向生成和完善宇宙机制模型的境界。符号回归（SR）和大语言模型（LLM）智能体的最新进展表明，这类系统能够从数据中恢复方程、融入领域先验知识，并自动化部分研究工作流。然而，大多数现有方法要么局限于狭窄的方程发现基准测试，要么专注于端到端的自动化流水线，而生物系统仍相对未被充分探索。在此，我们介绍MEDA系统，这是一个基于LLM和SR的智能体框架，用于发现生物及生物启发动态系统的常微分方程（ODE）模型。MEDA检索背景知识、定义允许变量、生成机制约束、提出候选ODE，并进行拟合与评估。我们在规范模型检索、基于推理的外推至未见变体以及开放发现等任务中对其进行评估，包括有实验数据和无实验数据的情况。在这些设置下，MEDA恢复了正确的状态变量，在检索和外推任务中实现了强大的结构恢复，并生成了生物学上可信的、面向发现的模型。消融和鲁棒性分析表明，知识引导的形式化和机制约束是承重组件，而仅靠数值拟合可以保留轨迹兼容但生物学不正确的方程。

Original abstract: Automatic scientific discovery has long been a goal of computational scholars - a machine that can discover nature's secrets on its own, moving computational systems beyond data-fitting tools toward the generation and refinement of mechanistic models of the universe. Recent advances in symbolic regression (SR) and large-language-model (LLM)-based agents suggest that such systems can recover equations from data, incorporate domain priors, and automate parts of the research workflow. However, most existing approaches either focus on narrow equation-discovery benchmarks or broad end-to-end automation pipelines, while biological systems remain comparatively underexplored. Here, we introduce the MEDA system, an LLM- and SR-powered agentic framework for discovering ordinary-differential-equation (ODE) models of biological and biologically inspired dynamical systems. MEDA retrieves background knowledge, defines admissible variables, generates mechanistic constraints, proposes candidate ODEs, and fits and evaluates them. We evaluate it across canonical model retrieval, reasoning-based extrapolation to unseen variants, and open-ended discovery, with and without experimental data. Across these settings, MEDA recovered the correct state variables, achieved strong structural recovery in retrieval and extrapolation tasks, and produced biologically plausible discovery-oriented models. Ablation and robustness analyses show that knowledge-guided formalization and mechanistic constraints are load-bearing components, whereas numerical fitting alone can preserve trajectory-compatible but biologically incorrect equations.

</details>


### [31] [STOCKTAKE: Measuring the Gap Between Perception and Action in LLM Agents with a Fair Oracle](https://arxiv.org/abs/2607.13618)
*Sagar Deb,Ashwanth Krishnan*

Main category: cs.AI

Selection reason: 提出STOCKTAKE benchmark，分离LLM agent的状态估计与控制，提供可迁移的agent评测方法。

TL;DR: 提出了STOCKTAKE基准，通过分离状态估计和控制失败来评估LLM智能体在多周决策任务中的表现，发现智能体在检测隐藏故障方面表现良好但技能分数差异大，失败包括反应不足和过度反应两面。


<details>
  <summary>Details</summary>
Motivation: 现有评估无法区分智能体失败的原因（误读世界 vs 知道但未行动），且缺乏公平的参考策略来单独衡量状态估计和控制。

Method: 构建了基于POMDP的26周供应链补货基准STOCKTAKE，包含六个隐藏因子过程，通过贝叶斯滤波器计算公平参考策略，并引入技能分数、检测延迟和知行率等指标来分别评估状态估计和控制。

Result: 四个智能体检测到84-88%的隐藏故障，技能分数从0.62到-0.23不等，两个低于症状盲基线；在诊断正确的压力周中仍有34-43%的缺货率，且该率与技能分数负相关。

Conclusion: 智能体失败有两面：反应不足和过度反应，STOCKTAKE能够有效测量这两个方向，揭示了知行差距的复杂性。

中文摘要：LLM智能体越来越多地在多周决策任务上被评估，其中驱动成本的状态从未被直接观测到。在此类任务上，最终成本无法说明智能体为何失败：它可能误读了世界，或者正确读取了世界但仍然未能行动（知行差距）。现有的评估无法分离这两种失败；它们的参考策略要么读取智能体从未看到的特权信息，要么完全缺失。我们引入了STOCKTAKE，一个为期26周的供应链补货基准，构建为一个具有六个隐藏因子过程的分块部分可观测马尔可夫决策过程，其设计使得公平的参考策略是可计算的：每个因子的精确贝叶斯滤波器驱动一个在智能体接收的相同观测流上的展开策略。将每次运行在症状盲基础库存下限（0）和该神谕（1）之间评分得到一个技能分数，并每周评分书面理由得到一个陈述信念检测延迟和一个知行率，从而分别测量状态估计和控制。在50个具有精心设计的压力曲线的种子上，Claude Sonnet 5、GPT-5.4、DeepSeek-V4-Pro和Grok 4.5检测到84-88%的隐藏故障，通常在故障发生一周内，但技能分数范围从0.62到-0.23：四个中的两个低于症状盲下限，同时命名因子的速度略快于击败下限的两个。失败有两面。在压力持续存在的情况下，每个模型在正确诊断的压力周中仍有34-43%以缺货结束，这一比率部分反映了模型注意到的周的严重性。该比率也与技能相反：低于下限的两个模型在诊断周缺货最少，因此反应不足只是差距的一面，它们的轨迹指向另一面，即成本超过其保护价值的响应。STOCKTAKE测量了该失败的两个方向。

Original abstract: LLM agents are increasingly evaluated on multi-week decision tasks in which the state that drives cost is never directly observed. On such tasks the final cost cannot say why an agent failed: it may have misread the world, or read it correctly and still failed to act (the knowing-doing gap). Existing evaluations cannot separate these two failures; their reference policies either read privileged information the agent never sees, or are missing altogether. We introduce STOCKTAKE, a 26-week supply-chain replenishment benchmark built as a factored partially observable Markov decision process with six hidden factor processes, designed so that a fair reference policy is computable: an exact Bayes filter per factor drives a rollout policy on the identical observation stream the agent receives. Scoring each run between a symptom-blind base-stock floor (0) and this oracle (1) yields a skill score, and grading each week's written rationale yields a stated-belief detection lag and a knowing-doing rate, so state estimation and control are measured separately. On fifty seeds with curated stress profiles, Claude Sonnet 5, GPT-5.4, DeepSeek-V4-Pro, and Grok 4.5 detect 84-88% of hidden failures, typically within a week of onset, yet span skill scores from 0.62 to -0.23: two of the four end below the symptom-blind floor while naming factors slightly faster than the two that beat it. The failure has two faces. Where stress persists, 34-43% of correctly diagnosed stress weeks still end in stockout for every model, a rate that partly reflects the severity of the weeks models notice. That rate also runs opposite to skill: the two models under the floor stock out least on diagnosed weeks, so under-response is only one face of the gap, and their traces point to the other, responses whose cost exceeds what they protect. STOCKTAKE measures both directions of that failure.

</details>


### [32] [Explaining Reinforcement Learning Agents via Inductive Logic Programming](https://arxiv.org/abs/2607.13655)
*Celeste Veronese,Edoardo Zorzi,Daniele Meli,Alessandro Farinelli*

Main category: cs.AI

Selection reason: 用ILP解释RL agent策略并提出可解释性度量，属于通用agent可解释性方法。

TL;DR: 本文提出了基于归纳逻辑编程的客观且面向规划的可解释性度量，用于量化强化学习策略的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有XRL主要依赖用户研究，缺乏共享评估指标；逻辑方法虽提供抽象但可解释性量化不足。

Method: 采用归纳逻辑编程提取符号规则，定义激活率、特征覆盖度、句法距离和语义距离等可解释性度量。

Result: 实验表明所提度量能揭示动作特定的学习动态、领域特征细粒度信息以及多智能体中的协作模式。

Conclusion: 所提度量有助于策略迁移和泛化。

中文摘要：可解释强化学习（XRL）旨在使强化学习（RL）策略更加透明和可解释，这在安全关键和以人为中心的场景中是一个关键需求。然而，它主要基于用户研究，因此针对特定受众的需求，缺乏共享的评估指标。另一方面，可解释人工智能（XAI）中基于逻辑的方法提供了决策的紧凑、人类可读的抽象。然而，逻辑表示的可解释性程度的系统量化仍然是一个开放问题。本研究旨在通过引入客观且面向规划的RL策略可解释性度量来推进XRL的最新技术。同时，它通过提供一种量化逻辑规则可解释性的原则性方法，超越了常识性评估和简单的命题片段，为XAI的逻辑领域做出贡献。我们采用归纳逻辑编程（ILP）来提取RL策略的符号表示，并定义了一组新颖的可解释性度量，包括激活率、特征覆盖度、句法距离和语义距离。这些度量量化了符号规则与智能体行为之间的对齐、特征在决策中的作用，以及训练过程中和单/多智能体RL中跨智能体的策略演化。跨不同RL领域的实验表明，所提出的度量揭示了超越全局回报的动作特定学习动态，提供了超越经典全局特征重要性估计方法的领域特征细粒度洞察，并揭示了MARL中的协调、专业化和适应模式。此外，它们为动作特定策略的迁移和泛化提供了关键见解。

Original abstract: Explainable Reinforcement Learning (XRL) seeks to make Reinforcement Learning (RL) policies more transparent and interpretable, a key requirement in safety-critical and human-centric scenarios. However, it is mostly based on user studies, thus targeting the needs of a specific audience and lacking shared evaluation metrics. On the other hand, logic-based approaches within eXplainable Artificial Intelligence (XAI) provide compact, human-readable abstractions of decision-making. However, the systematic quantification of the explainability degree of logical representations remains an open problem. This work aims to advance the state of the art in XRL by introducing objective and planning-oriented metrics for policy explainability in RL settings. At the same time, it contributes to the field of logic for XAI by providing a principled way to quantify the explainability of logical rules, moving beyond common-sense assessments and simple propositional fragments. We employ Inductive Logic Programming (ILP) to extract symbolic representations of RL policies and define a novel set of explainability metrics, including activation rate, feature coverage, syntactic distance and semantic distance. These metrics quantify alignment between symbolic rules and agent behavior, the role of features in decision-making, and the evolution of policies during training and across agents in single and multi-agent RL. Experiments across different RL domains show that the proposed metrics highlight action-specific learning dynamics beyond global return, provide fine-grained insights into domain features beyond classical approaches for global feature importance estimation, and uncover coordination, specialization, and adaptation patterns in MARL. Moreover, they provide crucial insights for the transfer and generalization of action-specific policies.

</details>


### [33] [CAVA: Canonical Action Verification and Attestation for Runtime Governance of Agentic AI Systems](https://arxiv.org/abs/2607.13716)
*Zexun Wang*

Main category: cs.AI

Selection reason: 提出CAVA运行时治理机制，确保agent动作可验证，属于agent安全控制。

TL;DR: 本文提出CAVA，一个运行时语义层，将异构代理动作转换为规范对象，解决AI治理中的动作验证问题，通过形式化和基准测试验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 代理AI系统中异构运行时导致动作记录不兼容，使治理难以追踪和验证批准、执行与证据之间的关联。

Method: 设计CAVA运行时语义层，将异构活动规范化为规范动作对象，结合语义模式检测、批准绑定、接收完整性等机制，并通过96种子384变体的基准测试评估。

Result: CAVA在语义等价、分离、抗篡改、可移植性和策略集成等维度表现良好，通过Azure部署验证了实际可用性。

Conclusion: 动作级规范化和策略可寻址语义模式是部署方AI治理的必要基础，CAVA提供了可行方案。

中文摘要：代理AI系统日益通过异构运行时运行：本地编码钩子、SDK工具、浏览器自动化、托管代理追踪、API网关和工作流引擎。因此，单个操作行为（如发布代码、更改身份状态、转移资金或导出数据）可能由许多不兼容的运行时记录表示。这使得一个基本的治理问题难以回答：实际批准了什么行动？什么证据将批准与执行绑定？独立验证者能否在以后重现相同的行动身份？
本文提出了规范动作验证与认证（CAVA），一个运行时语义层，用于将异构代理活动转换为规范运行时动作对象。CAVA位于证明携带代理动作（PCAA）之下：PCAA定义了部署者拥有的路由-审查-证明治理过程，而CAVA定义了该过程所治理的稳定动作对象。本文形式化了规范动作身份、语义模式检测、批准绑定、接收完整性、运行时可移植投影和可选的认证基底。我们通过一个包含96个种子、384个变体的基准测试研究了参考实现，覆盖语义等价、语义分离、包装器绕过、假阳性控制、批准绑定、接收可重现性、认证篡改检测、运行时可移植性、语义模式检测、策略退化以及Azure部署演练。本文的贡献在于将动作级规范化和策略可寻址语义模式作为部署方AI治理的必要基础进行了系统化表述。

Original abstract: Agentic AI systems increasingly act through heterogeneous runtimes: local coding hooks, SDK tools, browser automation, managed-agent traces, API gateways, and workflow engines. A single operational act such as publishing code, changing identity state, moving money, or exporting data may therefore be represented by many incompatible runtime records. This makes a basic governance question difficult to answer: what action was actually approved, what evidence binds the approval to execution, and can an independent verifier reproduce the same action identity later?
  This paper presents Canonical Action Verification and Attestation (CAVA), a runtime-semantics layer for converting heterogeneous agent activity into canonical runtime action objects. CAVA is positioned below Proof-Carrying Agent Actions (PCAA): PCAA defines the deployer-owned route-review-prove governance process, while CAVA defines the stable action object that process governs. The paper formalizes canonical action identity, semantic pattern detection, approval binding, receipt integrity, runtime-portable projection, and optional attestation substrates. We study a reference implementation through a 96-seed, 384-variant benchmark covering semantic equivalence, semantic separation, wrapper bypass, false-positive control, approval binding, receipt reproducibility, attestation tamper detection, runtime portability, semantic pattern detection, policy degradation, and Azure deployment drills. The contribution is a systems formulation of action-level canonicalization and policy-addressable semantic patterns as a necessary substrate for deployer-side AI governance.

</details>


### [34] [Experience Memory Graph: One-Shot Error Correction for Agents](https://arxiv.org/abs/2607.13884)
*Wenjun Wang,Yuchen Fang,Fengrui Liu,Zibo Liang,Kai Zheng*

Main category: cs.AI

Selection reason: 经验记忆图EMG用于agent一次性错误校正，基于轨迹复用，可迁移到coding agent。

TL;DR: 提出经验记忆图（EMG），将LLM代理的失败恢复建模为图匹配问题，通过从失败和成功的轨迹中提取公共子图与编辑路径，实现单次无循环的纠正，在ALFWorld和ScienceWorld上超过SOTA反思方法。


<details>
  <summary>Details</summary>
Motivation: 现有自我纠正机制依赖基于提示的反思，存在脆弱性、高开销和泛化困难的问题，需要更鲁棒且高效的失败恢复方法。

Method: 将训练轨迹（失败和成功）转化为有向动作决策图，通过图匹配提取公共子图（成功流程）和图编辑路径（修正动作），存入带有跨任务边的记忆图；测试时检索相关指导，实现单次无循环执行。

Result: 在ALFWorld和ScienceWorld上，EMG在成功率和平均奖励上持续优于最先进的反思基线，且无需测试时试错。

Conclusion: EMG通过图匹配有效解决LLM代理的失败恢复问题，避免试错循环，提升性能与效率。

中文摘要：大型语言模型（LLM）代理通过生成状态、动作和观察的顺序轨迹，在自主决策中展现出显著能力。然而，在复杂、长时程任务中，这些代理经常遭受累积误差，并且难以从失败中恢复。现有的自我纠正机制依赖于基于提示的反思，这本质上是脆弱的，由于迭代的试错循环而产生沉重的时间和API成本，并且产生可能难以泛化到新场景的任务特定记忆。为了解决这个问题，我们提出了经验记忆图（EMG），这是一个将代理失败恢复重新表述为图匹配问题的框架。在训练时，我们将失败的探索轨迹和成功的专家轨迹都转换为有向动作决策图。通过匹配这些图，我们提取公共子图（成功工作流）和图编辑路径，这些路径明确指示如何纠正失败（例如，在给定观察下应添加、删除或重新标记哪些动作），并将它们存储在具有任务内节点和跨任务边的记忆图中。在测试时，EMG检索相关见解并引导代理进行单次、无循环的执行。在ALFWorld和ScienceWorld上的实验表明，EMG在成功率和平均奖励方面持续优于最先进的反思基线，同时不需要测试时的试错。

Original abstract: Large Language Model (LLM) agents have shown remarkable capabilities in autonomous decision-making by generating sequential trajectories of states, actions, and observations. However, in complex, long-horizon tasks, these agents frequently suffer from compounding errors and struggle to recover from failures. Existing self-correction mechanisms rely on prompt-based reflection, which is inherently brittle, incurs heavy time and API costs due to iterative trial-and-error loops, and produces task-specific memory that may be hard to generalize to new scenarios. To address this, we propose Experience Memory Graph (EMG), a framework that reformulates agent failure recovery as a graph matching problem. At training time, we convert both failed exploration trajectories and successful expert trajectories into directed action decision graphs. By matching these graphs, we extract common subgraphs (successful workflows) and graph edit paths that explicitly indicate how to correct failures (e.g., which actions to add, delete, or relabel under a given observation), and store them in a memory graph with intra-task nodes and cross-task edges. At test time, EMG retrieves relevant insights and guides the agent in a single, loop-free execution. Experiments on ALFWorld and ScienceWorld show that EMG consistently outperforms state-of-the-art reflection baselines in success rate and average reward, while requiring no test-time trial-and-error.

</details>


### [35] [A Self-Evolving Agent for Longitudinal Personal Health Management](https://arxiv.org/abs/2607.13940)
*Haoran Li,Jiebi Deng,Tong Jin,Jinghong Han,Yuxin Wang,Zexin Wang,Qingyi Si,Weikang Gong,Xiahai Zhuang,Jia You,Wei Cheng,Jianfeng Feng,Hongcheng Guo*

Main category: cs.AI

Selection reason: 健康管理自进化agent架构，分离共享规则与私有记忆，体现通用记忆管理方法。

TL;DR: HealthClaw是一种开源智能体架构，通过分离共享知识与私有纵向记忆并在每次交互后归纳更新，在纵向健康支持中将准确率从0.2%提升至45.7%，同时减少上下文暴露和隐私风险。


<details>
  <summary>Details</summary>
Motivation: 现有健康AI系统孤立处理每次请求，无法利用个人历史信息提供个性化、不断更新的支持。

Method: 开发HealthClaw，其架构分离共享安全规则和医学知识与私有纵向记忆（档案事实、可重用程序、情节痕迹），并在每个情节后通过归纳机制决定更新内容。

Result: 在900个纵向支持探针中，准确率从0.2%升至45.7%，上下文暴露降低71.7%；在100个隐私探针中，隐私感知质量更高且不安全感更少；在生物医学任务中，主要指标平均绝对增益27.0个百分点。

Conclusion: 离线基准支持受控、自进化记忆用于纵向个人健康智能体，但临床有效性需前瞻性评估。

中文摘要：个人健康管理在反复的接触中展开，然而大多数健康AI系统孤立地处理每个请求。我们开发了HealthClaw，一种开源智能体架构，它能够随着个人的日常习惯、偏好、测量数据和风险的变化更新支持。它将共享的安全规则和医学知识与私有的纵向记忆分开，后者包含个人档案事实、可重用程序和情节痕迹。在每个情节之后，归纳机制决定哪些内容应更新档案、修改程序、保留为情节或被排除。我们使用一个合成的一年期基准和九个200例生物医学任务评估了HealthClaw。在900个纵向支持探针中，答案准确率从当前查询提示的0.2%提高到HealthClaw的45.7%，同时提示端上下文暴露比全历史提示低71.7%。在100个隐私探针中，HealthClaw产生了比两个基线更高的隐私感知答案质量和更少的不安全披露。在生物医学任务中，任务特定主要指标的平均绝对增益为27.0个百分点，其中七个增益在错误发现率校正后仍显著。这些离线基准支持了纵向个人健康智能体的受控、自进化记忆，尽管临床有效性需要前瞻性评估。HealthClaw可在https://github.com/HC-Guo/HealthClaw上公开获取。

Original abstract: Personal health management unfolds over repeated encounters, yet most health AI systems treat each request in isolation. We developed HealthClaw, an open-source agent architecture that updates support as a person's routines, preferences, measurements and risks change. It separates shared safety rules and medical knowledge from private longitudinal memory containing profile facts, reusable procedures and episodic traces. After each episode, induction determines what should update the profile, revise a procedure, remain episodic or be excluded. We evaluated HealthClaw with a synthetic year-long benchmark and nine 200-case biomedical tasks. Across 900 longitudinal support probes, answer accuracy increased from 0.2% with current-query prompting to 45.7% with HealthClaw, while prompt-side context exposure was 71.7% lower than with full-history prompting. In 100 privacy probes, HealthClaw produced higher privacy-aware answer quality and fewer unsafe disclosures than both baselines. Across the biomedical tasks, the mean absolute gain in the task-specific primary metric was 27.0 percentage points, and seven gains remained significant after false-discovery-rate correction. These offline benchmarks support governed, self-evolving memory for longitudinal personal health agents, although clinical effectiveness requires prospective evaluation. HealthClaw is publicly available at https://github.com/HC-Guo/HealthClaw.

</details>


### [36] [Do Agent Optimizers Compound? A Continual-Learning Evaluation on Terminal-Bench 2.0](https://arxiv.org/abs/2607.14004)
*Wenxiao Wang,Priyatham Kattakinda,Soheil Feizi*

Main category: cs.AI

Selection reason: 研究agent优化器在持续学习中的复合收益，直接关联agent优化与长期部署评估。

TL;DR: 研究发现，只有通过内置回归控制的优化方法才能实现智能体优化增益的复合，在持续学习场景中持续提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体优化方法通常是一次性评估，未考虑实际部署中递归优化的场景，因此需要研究优化增益是否能在新任务上复合。

Method: 在Terminal-Bench 2.0上构建两阶段持续学习评估，比较GEPA、Meta Harness和RELAI-VCL三种方法在相同优化预算下的表现。

Result: RELAI-VCL是唯一实现正向迁移和持续改进的方法，终身平均通过率76.4%，显著高于其他方法（GEPA 66.0%，Meta Harness 64.6%，基线58.7%）。

Conclusion: 优化增益复合的关键在于优化循环中内置回归控制，以避免短路解决方案并促进泛化。

中文摘要：大多数来自智能体优化方法的报告增益都是一次性的：智能体针对固定基准进行优化，然后将所得改进报告为该方法的一种稳定特性。这没有测试对于部署的智能体而言重要的场景，即随着时间推移出现新的失败和新任务时，优化被递归应用。由此提出的核心问题是优化器驱动的增益是否会复合：在一次优化智能体后，能否再次针对新到达的任务进行优化而不侵蚀第一轮产生的增益？我们通过一个两阶段持续学习评估来研究这个问题，该评估基于Terminal-Bench 2.0中的困难任务，在相同优化预算下比较了三种智能体框架优化方法（GEPA、Meta Harness和RELAI的可验证持续学习，RELAI-VCL）。在传统的、静态的、单阶段设置中，所有三种方法都优于基线智能体。然而，一旦引入新任务，这些方法就出现明显分歧：GEPA优化的智能体迁移性能低于未优化的基线，Meta Harness迁移良好但在获得第二次优化预算后未能进一步改进，而RELAI-VCL是唯一一种既能正向迁移到未见任务，又能在这些任务被纳入优化目标后继续改进的方法，在每个评估阶段都达到最高通过率，并且整体终身平均通过率最高（76.4%，而GEPA为66.0%，Meta Harness为64.6%，基线为58.7%）。我们的关键观察是，只有当回归控制被内置到优化循环中时，优化增益才能复合，从而提供一种对无法泛化的捷径解决方案的归纳偏差。

Original abstract: Most reported gains from agent-optimization methods are one-shot: an agent is optimized against a fixed benchmark and the resulting improvement is reported as if it were a stable property of the method. This does not test the setting that matters for deployed agents, where optimization is applied recursively as new failures and new tasks appear over time. The central question this raises is whether optimizer-driven gains compound: after an agent has been optimized once, can it be optimized again on newly arrived tasks without eroding the gains the first round produced? We study this question with a two-phase continual-learning evaluation built from hard tasks in Terminal-Bench 2.0, comparing three approaches to agent-harness optimization (GEPA, Meta Harness, and RELAI's Verifiable Continual Learning, RELAI-VCL) under identical optimization budgets. All three methods improve over the baseline agent in the conventional, static, single-phase setting. However, once new tasks are introduced, the methods diverge sharply: GEPA's optimized agent transfers below the unoptimized baseline, Meta Harness transfers well but fails to improve further once given a second optimization budget, and RELAI-VCL is the only method that both transfers positively to unseen tasks and continues improving after those tasks are folded into the optimization objective, reaching the highest pass rate at every evaluated stage and the highest lifelong average pass rate overall (76.4% vs. 66.0% for GEPA, 64.6% for Meta Harness, and 58.7% for the baseline). Our key observation was that optimization gains compounded only when regression control was built into the optimization loop, providing an inductive bias against shortcut solutions that fail to generalize.

</details>


### [37] [Deep Interaction: An Efficient Human-AI Interaction Method for Large Reasoning Models](https://arxiv.org/abs/2607.14049)
*Hefeng Zhou,Jinxuan Zhang,Jiong Lou,Yuxin Liu,Chaochao Lu,Jingjing Qu,Jie Li*

Main category: cs.AI

Selection reason: 提出Deep Interaction，允许直接编辑CoT错误，改进agent交互与纠错机制。

TL;DR: 提出深度交互方法，通过直接编辑错误步骤并提炼为精简提示，实现LLM推理错误的高效纠正，在STEM任务上纠正成功率提升25%以上，令牌使用减少40%。


<details>
  <summary>Details</summary>
Motivation: 当前交互方法在纠正错误时效率低下：重新生成可能重复犯错，用户手动标记后模型仍可能重犯。

Method: 允许直接编辑原始响应中的错误部分，保留正确步骤，将编辑后的CoT提炼为精简提示，引导LLM沿正确路径推理。

Result: 在STEM任务上，纠正成功率提升超过25%，令牌使用减少约40%。

Conclusion: 深度交互是一种高效的人机交互方法，能显著提高错误纠正效率并节省计算资源。

中文摘要：链式思维推理的出现显著增强了大型语言模型处理复杂多步任务的能力。然而，当出现错误时，当前的交互方法通常涉及重新生成另一个可能再次出错的响应，或者用户费力地在后续轮次中标出错误步骤，而模型可能回应<你说得对，我在这里犯了错误>，随后重复类似错误。为了解决这个问题，我们提出了一种高效的人工干预机制，用于精确纠正LLM中的推理错误，称为深度交互。我们的方法能够直接编辑原始响应，允许纠正错误部分的同时保留准确的推理步骤。我们将编辑后的CoT提炼成精简提示，进而引导LLM沿着修正的推理路径前进。实验结果显示，与基线方法相比，我们的方法在STEM任务推理上纠正成功率提高了超过25%，且令牌使用减少了约40%。

Original abstract: The emergence of Chain-of-Thought (CoT) reasoning has significantly enhanced the ability of large language models (LLMs) to tackle complex, multi-step tasks. However, when errors occur, current interaction approaches typically involve re-generating another response that may make mistakes again, or users laboriously flag the faulty step in follow-up turns that may get responses <You are right, I made a mistake here> followed by similar errors recurring. To address this issue, we propose an efficient human intervention mechanism for precisely correcting reasoning errors in LLMs, termed Deep Interaction. Our approach enables direct editing of the original response, allowing erroneous parts to be corrected while preserving accurate reasoning steps. We refine the edited CoT into a distilled prompt, which then steers the LLM along the corrected reasoning path. Experimental results show that our method achieves over a 25% improvement in correction success rate and reduces token usage by approximately 40% on STEM tasks reasoning compared to baseline approaches.

</details>
